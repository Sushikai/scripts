"""R152 mobile .bv-sector-pill-chg 板块涨幅 9→10.5px — 聚合条最后一个 9px 数据 chip.

原: R86 板块聚合 pill 的涨幅小字 (.bv-sector-pill-chg) font 9px — 聚合条"一眼扫完
    最强板块"的核心数字 (板块红/绿 + 涨幅), 9px 是全页最后一个低于 chip 地板 10.5px
    的数据 chip (R148 bv-vr / R149 sector-chg 已收, R152 收尾聚合条)。
R152: 9→10.5px (chip 地板统一), 跟 pill-name 11px 保持 0.5px 层级差。
第一性原理: 聚合条的意义是"扫一眼哪个板块最强" (R86), 涨幅数字就是"温度计读数" —
    用户靠它决定点哪个板块 pill。9px 读不清 = 聚合条失去意义。
    所有数据 chips 统一 10.5px 地板后, 全页 0 个 9px (R148-R152 闭环)。
R86 守护: weight 700 + 颜色 (bv-pos red / bv-neg green) 不动。
断言 (mock 数据, 390px, 有 sector-bar):
  - sector-pill-chg font = 10.5px (was 9)
  - font-weight 700 保留
...
async def run():
    async with async_playwright() as p:
        ...
        m = await page.evaluate(r"""() => {
          ...
          var chg = bar.querySelector('.bv-sector-pill-chg');
          var name = bar.querySelector('.bv-sector-pill-name');
          if (!chg) return {note:'no pill-chg'};
          var cs = getComputedStyle(chg);
          return {
        ...
        assert m.get("fontSize") == "10.5px", f"sector-pill-chg should be 10.5px (was 9), got {m.get('fontSize')}"
        assert m.get("fontWeight") == "700", f"sector-pill-chg weight regression: {m.get('fontWeight')}"
        # 层级差: chg 10.5 < name 11
        assert m.get("nameFont") == "11px", f"sector-pill-name should stay 11px, got {m.get('nameFont')}"

        await browser.close()
        print(f"[OK] R152 sector-pill-chg — 10.5px (was 9) | weight 700 + name 11 (层级差 ✓)")
