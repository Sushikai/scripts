#!/usr/bin/env python3
"""
test_strat_era43_pvi.py
Ship 99 单元测试 — PVI (Positive Volume Index)
"""
import sys
import unittest
...
from tuixue_v3.strat_era43_pvi import (
    PVISignal, compute_pvi_series,
    generate_signal, screen_universe, split_buy_sell, summarize,
)


class TestComputePVISeries(unittest.TestCase):
    def test_basic(self):
        n = 25
        closes = [10.0 + i * 0.5 for i in range(n)]
        vols = [1000.0 + i for i in range(n)]
        s = compute_pvi_series(closes, vols)
        self.assertEqual(len(s), n)
        self.assertEqual(s[0], 100.0)

    def test_short(self):
        self.assertEqual(compute_pvi_series([10.0], [1000.0]), [])
