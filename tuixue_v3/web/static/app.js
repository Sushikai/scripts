...
// $ 和 $$ 已在 core.js 里 const 声明;删掉 app.js 副本避免重复声明 SyntaxError
// (core.js 加载顺序在前,这里直接复用全局)
...
document.addEventListener('click', (e) => {
  ...
  if (!action) return;
  // R20: data-stop="1" 阻止冒泡触发父按钮 (例如搜索结果 pill 里的 dexin-check 不应同时跳转个股页)
  ...
  try {
    switch (name) {
      ...
      case 'dexin-check':
        // 单股走 DexinTrendAgent 判定, 弹 modal 显示时序链条
        dexinCheckOne(arg);
        break;
...
var API_TIMEOUTS = {
  ...
  '/api/zt/':             120_000,  // ZT 策略回测/优化 (缓存预热 7-30s, 优化 5+ min)
...
// R21: 全局限流 — 防止 retry 风暴 / 用户疯狂点按钮触发 server 雪崩
// 用法: if (!_rateGate('stock', 5, 1000)) return;  // 同一 key 1s 最多 5 次
var _rateGateMap = {};  // { key: [ts, ts, ...] }
function _rateGate(key, max, windowMs) {
  const now = Date.now();
  const arr = (_rateGateMap[key] || []).filter(t => now - t < windowMs);
  if (arr.length >= max) return false;
  arr.push(now);
  _rateGateMap[key] = arr;
  return true;
}

// R24: 请求合并 — 同一 (path, options) 在 200ms 内多次触发,共用同一个 in-flight promise
// 解决: 多 view 切换时仪表盘 signal 同一 URL 3 个并发 → 改成 1 个
var _inflightFetches = new Map();  // key -> { ts, promise }
var _INFlightWindowMs = 200;
function _inflightKey(path, opts) {
  // GET + 类似 query 视为同一 key (忽略 signal)
  const m = opts?.method || 'GET';
  return m + ' ' + path + ' ' + (opts?.body || '');
}
function _getInflight(key) {
  const e = _inflightFetches.get(key);
  if (!e) return null;
  if (Date.now() - e.ts > _INFlightWindowMs) {
    _inflightFetches.delete(key);
    return null;
  }
  return e.promise;
}
function _setInflight(key, p) {
  _inflightFetches.set(key, { ts: Date.now(), promise: p });
  p.finally(() => {
    const cur = _inflightFetches.get(key);
    if (cur && cur.promise === p) _inflightFetches.delete(key);
  });
}
// R-fix-2026-08-01: 切页/切股 abort 全部 in-flight + SSE — 避免旧 view 数据
// 污染新 view、HTTP/1.1 6 连接池被旧请求占满
// (跟 view-leave hook 各 view 单独 cleanup 互补,这里是兜底)
function _inflightAbortAll() {
  try {
    if (typeof _stockFetchCtrl !== 'undefined' && _stockFetchCtrl) {
      try { _stockFetchCtrl.abort(); } catch (e) {}
    }
    if (typeof _stockAuxAborters !== 'undefined' && _stockAuxAborters) {
      Object.values(_stockAuxAborters).forEach(c => { try { c.abort(); } catch (e) {} });
    }
    if (typeof window._stockInflightAborter !== 'undefined' && window._stockInflightAborter) {
      try { window._stockInflightAborter.abort(); } catch (e) {}
    }
    if (typeof _reviewState !== 'undefined' && _reviewState?._inflightAborter) {
      try { _reviewState._inflightAborter.abort(); } catch (e) {}
    }
    if (typeof _prefetchAbortAll === 'function') {
      try { _prefetchAbortAll(); } catch (e) {}
    }
  } catch (e) { console.warn('inflightAbortAll:', e); }
  _inflightFetches.clear();
}
function _abortAllSSE() {
  try {
    if (typeof _SSE_LIST !== 'undefined' && _SSE_LIST) {
      _SSE_LIST.forEach(s => { try { s.close(); } catch (e) {} });
      _SSE_LIST.length = 0;
    }
  } catch (e) { console.warn('abortAllSSE:', e); }
}
window._inflightAbortAll = _inflightAbortAll;
window._abortAllSSE = _abortAllSSE;
...
async function _fetchWithTimeout(path, opts = {}) {
  ...
  // R23: 离线时直接抛错,跳过 retry (避免 retry 队列堆积 60s+)
...
var _ZT_CHAIN_CACHE_MAX = 500;   // B-14: LRU 上限
...
// B-14: 写入时检查容量, 超限按插入顺序淘汰最久未访问
function _ztChainCacheSet(code, val) {
  if (_ztChainCache.size >= _ZT_CHAIN_CACHE_MAX) {
    // 1) 先清过期
    const now = Date.now();
    for (const [k, v] of _ztChainCache) {
      if (now - v.ts > _ZT_CHAIN_CACHE_TTL_MS) _ztChainCache.delete(k);
    }
    // 2) 还超就淘汰最早插入 (Map 保序)
    if (_ztChainCache.size >= _ZT_CHAIN_CACHE_MAX) {
      const n_to_drop = _ztChainCache.size - _ZT_CHAIN_CACHE_MAX + 1;
      const it = _ztChainCache.keys();
      for (let i = 0; i < n_to_drop; i++) {
        const k = it.next().value;
        if (k != null) _ztChainCache.delete(k);
      }
    }
  }
  _ztChainCache.set(code, val);
}
...
async function _ztChainFetch(codes) {
      ...
      _ztChainPendingPromise = (async () => {
        try {
          ...
          for (const [code, rows] of Object.entries(merged)) {
            _ztChainCacheSet(code, { ts: Date.now(), rows: rows || [] });
          }
          return merged;
        } finally {
  ...
  for (const c of todo) if (!out[c] || out[c].length) _ztChainCacheSet(c, { ts: Date.now(), rows: out[c] || [] });
  return out;
}
...
// ────────────────────────────────────────────
// R76: 长任务监控 — 单个 task > 100ms 警告,提示阻塞主线程
// ────────────────────────────────────────────
    ...
    const po = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        if (entry.duration > 100) {
          console.warn(`[long-task] ${entry.duration.toFixed(0)}ms`, entry.name || '(anonymous)');
        }
      }
    });
...
window.addEventListener('error', (e) => {
  if (e.target && e.target.tagName) return; // 只处理 JS 错误,不处理资源加载
  console.error('[global error]', e.error || e.message);
  if (typeof toast === 'function') toast('脚本错误: ' + String(e.message || '').slice(0, 80), 'error', 3000);
});
...
function _drainToast() {
  ...
  _toastActive = true;
  // R12: error toast 可点击消失,消息显示为 HTML
  const isErr = next.kind === 'error';
  toastEl.innerHTML = isErr
    ? `<span class="toast-err-msg">${escapeHtml(next.msg)}</span><span class="toast-dismiss">✕</span>`
    : escapeHtml(next.msg);
  toastEl.className = `toast toast-${next.kind} show`;
  toastEl.hidden = false;
  toastEl.onclick = isErr ? () => {
    toastEl.hidden = true;
    _toastActive = false;
    _drainToast();
  } : null;
  ...
  setTimeout(_hide, next.ms);
}
function toast(msg, kind = 'info', ms = 2400) {
  if (!toastEl) return;
  if (kind === 'error') ms = Math.max(ms, 4000);
  _toastQueue.push({ msg, kind, ms });
  _drainToast();
}
// R12: showToast 是 toast 别名 (UI 里混用,统一)
window.showToast = toast;

// R1000-B1: 跨模块自选共享 — 所有 view 复用同一套 wlToggle 逻辑
window._wlCodeSet = null;
window.wlGetCodes = async function () {
  if (window._wlCodeSet) return window._wlCodeSet;
  try {
    const data = await api('/api/watchlist');
    const items = (data && data.items) || [];
    window._wlCodeSet = new Set(items.map(function (x) { return x.code; }));
  } catch (_) { window._wlCodeSet = new Set(); }
  return window._wlCodeSet;
};
window.wlToggle = async function (code, name) {
  var codes = await window.wlGetCodes();
  if (codes.has(code)) {
    try {
      await api('/api/watchlist/' + encodeURIComponent(code), { method: 'DELETE' });
      codes.delete(code);
      toast('✓ ' + code + ' 已移出自选', 'success', 2200);
      document.dispatchEvent(new CustomEvent('watchlist-changed', { detail: { action: 'remove', code: code, name: name } }));
      return false;
    } catch (e) { toast('✗ 删除失败: ' + e.message, 'error', 3000); return true; }
  } else {
    try {
      await api('/api/watchlist', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code: code, name: name || code }) });
      codes.add(code);
      toast('✓ ' + code + ' 已加入自选', 'success', 2200);
      document.dispatchEvent(new CustomEvent('watchlist-changed', { detail: { action: 'add', code: code, name: name } }));
      return true;
    } catch (e) { toast('✗ 加入失败: ' + e.message, 'error', 3000); return false; }
  }
};
window.wlRefreshBtn = async function (btn) {
  var code = btn.dataset.wlCode;
  if (!code) return;
  var codes = await window.wlGetCodes();
  btn.textContent = codes.has(code) ? '✓' : '⭐';
  btn.title = codes.has(code) ? '移出自选' : '加入自选';
};

// ────────────────────────────────────────────
// dexin 单股判定 modal — 全 A / dash / 自选 任意代码都可点开看时序链条
// 走 /api/dexin/check/{code}, 命中 DexinTrendAgent.detect, 显示 5 阶段进度 + 建议
// ────────────────────────────────────────────
const DEXIN_PHASE_LABEL = {
  no_uptrend: '非上升趋势',
  none:       '暂无形态',
  cang_zha:   '藏诈诱多',
  xu_sha:     '虚杀洗盘',
...
const DEXIN_PHASE_VARIANT = {
  benign:    '✓ 良性',
  dangerous: '⚠ 深度回撤',
};
async function dexinCheckOne(code) {
  if (!code || !/^\d{1,6}$/.test(code)) {
    toast('✗ 代码格式错误', 'error');
    return;
  }
  const modal = _openDexinModal(code, '加载时序链条…');
  try {
    const r = await _fetchWithTimeout('/api/dexin/check/' + encodeURIComponent(code), { timeout: 12000 });
    const j = await r.json();
    if (!j.ok) {
      _setDexinModalError(modal, j.error || '判定失败');
      return;
    }
    _renderDexinModal(modal, j.data || {});
  } catch (e) {
    if (!String(e.message || '').includes('abort')) {
      _setDexinModalError(modal, '请求失败: ' + (e.message || '未知错误'));
    }
  }
}

function _openDexinModal(code, loadingText) {
  document.querySelectorAll('.dexin-check-modal').forEach(m => m.remove());
  const overlay = document.createElement('div');
  overlay.className = 'dexin-check-modal';
  overlay.dataset.dexinCode = code;
  overlay.innerHTML = `
    <div class="dexin-modal-card" role="dialog" aria-modal="true">
      <button class="dexin-modal-close" aria-label="关闭">✕</button>
      <div class="dexin-modal-header">
        <span class="dexin-modal-code">${escapeHtml(code)}</span>
        <span class="dexin-modal-name" data-dexin-name>—</span>
        <span class="dexin-modal-phase" data-dexin-phase>·</span>
      </div>
      <div class="dexin-modal-body" data-dexin-body>
        <div class="dexin-modal-loading">${escapeHtml(loadingText || '加载中…')}</div>
      </div>
      <div class="dexin-modal-footer" data-dexin-footer hidden>
        <button class="btn-mini" data-dexin-jump>跳转得鑫量变术</button>
        <button class="btn-mini" data-dexin-add-wl>⭐ 加入自选</button>
      </div>
    </div>
  `;
  document.body.appendChild(overlay);
  overlay.addEventListener('click', (e) => {
    if (e.target === overlay || e.target.classList.contains('dexin-modal-close')) {
      overlay.remove();
    }
  });
  overlay.querySelector('[data-dexin-jump]').addEventListener('click', () => {
    overlay.remove();
    if (typeof showView === 'function') showView('dexin');
  });
  overlay.querySelector('[data-dexin-add-wl]').addEventListener('click', async (e) => {
    const code = overlay.dataset.dexinCode;
    const name = overlay.querySelector('[data-dexin-name]').textContent || code;
    const added = await window.wlToggle(code, name);
    if (added) e.target.textContent = '✓ 已加入';
  });
  return overlay;
}

function _setDexinModalError(modal, msg) {
  const body = modal.querySelector('[data-dexin-body]');
  body.innerHTML = `<div class="dexin-modal-error">⚠ ${escapeHtml(msg)}</div>`;
}

function _renderDexinModal(modal, data) {
  const code = modal.dataset.dexinCode;
  const phase = data.phase || 'none';
  const variant = data.variant || 'benign';
  const phaseLabel = DEXIN_PHASE_LABEL[phase] || phase;
  const variantLabel = DEXIN_PHASE_VARIANT[variant] || '';
  modal.querySelector('[data-dexin-name]').textContent = data.name || code;
  const phaseEl = modal.querySelector('[data-dexin-phase]');
  phaseEl.textContent = phaseLabel + (variant === 'dangerous' ? ' · ' + variantLabel : '');
  phaseEl.dataset.phase = phase;
  phaseEl.dataset.variant = variant;
  const dates = data.phase_dates || {};
  const chips = [];
  // R20-fix: 后端 phase_dates 用中文 key ("藏诈日"/"虚杀日"/"洗盘区间"/"得鑫日"), 同时兼容英文别名
  const fraud = dates['藏诈日'] || dates.fraud_date;
  const kill  = dates['虚杀日'] || dates.kill_date;
  const range = dates['洗盘区间'] || dates.kill_range;
  const gain  = dates['得鑫日'] || dates.gain_date;
  const cycle = dates['cycle_days'] || dates['周期天数'];
  if (fraud) chips.push(`<span class="dx-phase-chip" data-kind="fraud">藏诈 ${escapeHtml(fraud)}</span>`);
  if (kill)  chips.push(`<span class="dx-phase-chip" data-kind="kill">虚杀 ${escapeHtml(kill)}</span>`);
  if (range) chips.push(`<span class="dx-phase-chip" data-kind="range">洗盘 ${escapeHtml(range)}</span>`);
  if (gain)  chips.push(`<span class="dx-phase-chip" data-kind="gain">得鑫 ${escapeHtml(gain)}</span>`);
  if (cycle) chips.push(`<span class="dx-phase-chip" data-kind="cycle">周期 ${cycle} 天</span>`);
  const body = modal.querySelector('[data-dexin-body]');
  body.innerHTML = `
    <div class="dexin-modal-advice">${escapeHtml(data.advice || '')}</div>
    ${chips.length ? `<div class="dx-phase-row">${chips.join('')}</div>` : ''}
    ${data.creed_line ? `<div class="dexin-modal-creed">"${escapeHtml(data.creed_line)}"</div>` : ''}
  `;
  if (data.signal) {
    const signalTag = document.createElement('div');
    signalTag.className = 'dexin-modal-signal';
    signalTag.textContent = '✓ 触发得鑫买点';
    body.prepend(signalTag);
  }
  modal.querySelector('[data-dexin-footer]').hidden = false;
}

// R14-R15: 统一 empty / error 卡片组件
function emptyCard(msg = '暂无数据', icon = '—') {
  return `<div class="empty-state"><span class="empty-icon">${icon}</span><span class="empty-msg">${escapeHtml(msg)}</span></div>`;
}
function errorCard(msg, onRetry) {
  const retryBtn = onRetry ? `<button class="btn-mini primary" onclick="(${onRetry.toString()})()">重试</button>` : '';
  return `<div class="empty-state error"><span class="empty-icon">⚠</span><span class="empty-msg">${escapeHtml(msg)}</span>${retryBtn}</div>`;
}

// R20: 全局 dexin-loaded 监听 — 把 phase 信息 patch 到全 A 行内 badge
document.addEventListener('dexin-loaded', function(ev){
  var map = (ev && ev.detail && ev.detail.map) || window.__dexinPhaseMap || {};
  var cells = document.querySelectorAll('#as-stocks-tbody td[data-col="得鑫"]');
  cells.forEach(function(td){
    var code = td.dataset.dxinCode;
    var info = map[code];
    var badge = td.querySelector('.dxin-row-badge');
    if (!badge) return;
    if (info) {
      badge.dataset.phase = info.phase;
      badge.dataset.variant = info.variant;
      badge.textContent = info.phase === 'de_xin' ? '✓得' :
                          info.phase === 'clearing' ? '等破' :
                          info.phase === 'xu_sha'   ? '洗盘' :
                          info.phase === 'cang_zha' ? '藏诈' : '验';
      badge.title = info.phase + (info.cycle ? ' · 周期 ' + info.cycle + ' 天' : '') +
                    (info.variant === 'dangerous' ? ' · ⚠ 危险回撤' : '') +
                    ' · 点击触发 DexinTrendAgent 单股判定';
    } else {
      // 不在 dexin 候选池 → 还原成 "验" 提示
      if (badge.dataset.phase) {
        delete badge.dataset.phase;
        delete badge.dataset.variant;
        badge.textContent = '验';
        badge.title = '点击行触发得鑫量变术 5 阶段判定';
      }
    }
  });
});
...
function _routeFromHash() {
  // R31: search param `?view=screener` 优先 (btCopyShareUrl 用的格式)
  ...
  // R-fix-2026-07-26: hash 路由到 sector 时把 arg 装到 ctx.name,否则 renderSectorDetail 拿到空 ctx 永远 "未指定板块"
  // showView(name, ctx) — 第二参就是 ctx(也用 opts.push),_runViewEntry 转发给 renderSectorDetail
  if (name === 'sector' && arg) {
    showView(name, { name: arg, push: false });
  } else {
    showView(name, { push: false });
  }
  // R32: 解析 hash 内的 bt params, 自动触发回测 (深链体验)
  if (name === 'screener') {
    try {
      const hashStr = h.includes('&') ? h.split('&').slice(1).join('&') : h.replace(/^bt/, '');
      const hashParams = new URLSearchParams(hashStr);
      const _applyRange = (id, key) => {
      ...
      _applyRange('bt-breadth', 'p0');
      _applyRange('bt-breadth-soft', 'p1');
      _applyRange('bt-sector-hot', 'p2');
      _applyRange('bt-sector-inflow', 'p3');
      _applyRange('bt-tail-vol-min', 'tail');
      const idx = hashParams.get('idx');
      const sec = hashParams.get('sec');
      if (idx === '1') { const e = document.getElementById('bt-index-late-up'); if (e) e.checked = true; }
      if (sec === '1') { const e = document.getElementById('bt-sector-late-up'); if (e) e.checked = true; }
      // R32: 有 run id 自动触发 (延迟 800ms 等 view 渲染完)
      const run = hashParams.get('run');
      if (run) {
        setTimeout(() => { try { window.btStart && window.btStart(); } catch (e) { console.warn('[deep-link btStart]', e); } }, 800);
      }
...
function _dashLoadPhased() {
  ...
  if (cached) {
    if (cached.signal && typeof _paintSignalCol === 'function') {
    ...
    if (cached.hot && typeof _paintHotSectors === 'function') {
...
async function _dashRefreshSignal() {
  if (typeof _paintSignalCol !== 'function') {
    // view-dash.js 还没加载完,跳过 (上层 onload 或重新触发会再调)
    return;
  }
  try {
...
async function _dashRefreshHot() {
  if (typeof _paintHotSectors !== 'function') return;
  try {
...
function _paintHotSectors(d) {
  ...
  host.innerHTML = tiles.map(t => {
    ...
    const ztBadge = ztN > 0 ? '<span class="hs-tile-zt" title="该板块涨停数">' + ztN + '家涨停</span>' : '';
    // R1000-B4: 开盘啦对齐 — L1 色点 + L2 申万分类
    const tx = t.taxonomy || {};
    const l1Dot = tx.l1 && tx.l1_color
      ...
      : '';
    const l2Label = tx.l2 ? '<span class="hs-tile-l2 dim">' + escapeHtml(tx.l2) + '</span>' : '';
    const boardChip = tx.board ? '<span class="hs-tile-board chip chip-mute">' + escapeHtml(tx.board) + '</span>' : '';
...
// R11: 维护 _STOCK_CACHE_KEY_PREFIX.* 的 LRU 索引,防止 sessionStorage 写爆 5MB quota
var _STOCK_CACHE_MAX = 80;  // 80 个 (code,date) 槽位,够 5 只股 × 16 天 or 80 只股 × 今日
var _stockCacheIndex = null;  // ['code_date', ...] LRU 顺序
function _stockCacheLoadIndex() {
  if (_stockCacheIndex) return _stockCacheIndex;
  try {
    const raw = sessionStorage.getItem(_STOCK_CACHE_KEY_PREFIX + '__index__');
    _stockCacheIndex = raw ? JSON.parse(raw) : [];
  } catch { _stockCacheIndex = []; }
  return _stockCacheIndex;
}
function _stockCacheSaveIndex() {
  try { sessionStorage.setItem(_STOCK_CACHE_KEY_PREFIX + '__index__', JSON.stringify(_stockCacheIndex)); } catch {}
}
function _stockCachePrune() {
  const idx = _stockCacheLoadIndex();
  while (idx.length > _STOCK_CACHE_MAX) {
    const old = idx.shift();
    try { sessionStorage.removeItem(_STOCK_CACHE_KEY_PREFIX + old); } catch {}
  }
}
function _stockCacheTouch(key) {
  const idx = _stockCacheLoadIndex();
  const i = idx.indexOf(key);
  if (i >= 0) idx.splice(i, 1);
  idx.push(key);
  if (idx.length > _STOCK_CACHE_MAX) _stockCachePrune();
  _stockCacheSaveIndex();
}
function _stockCacheSave(code, date, data) {
  try {
    const key = _stockCacheKey(code, date);
    const suffix = code + '_' + (date || 'today');
    sessionStorage.setItem(key, JSON.stringify({ ts: Date.now(), data }));
    _stockCacheTouch(suffix);
  } catch (e) { /* quota — 静默丢,下次读会穿透 */ }
}
function _stockCacheLoad(code, date) {
  try {
    ...
    // R11: 命中也算一次访问,刷新 LRU
    _stockCacheTouch(code + '_' + (date || 'today'));
    return p.data;
  } catch (e) { return null; }
}
...
function renderBacktestSector(sectors) {
  ...
  if (!host) return;
  // B-17: 防御 sectors 非 array (后端 schema 漂移)
...
async function _addHist(code, name) {
  ...
  try {
    ...
    await _loadHist();
    _renderHist();
    // R37: 用户浏览过这只 → 后台 idle 时预拉其它历史 top N,下次切秒开
  ...
  list.querySelectorAll('.sh-pill').forEach(p => {
    p.addEventListener('click', (e) => {
      ...
      if (c) {
        ...
        loadStockDetail(c);
        // R37: 鼠标 hover 历史股 200ms 后预拉 — 用户切走几乎秒开
        clearTimeout(_prefetchHoverT);
        _prefetchHoverT = setTimeout(() => _prefetchOne(c), 200);
      }
    });
  });
}
...
var _prefetchInflight = new Map();  // code -> AbortController (R-fix-2026-08-01: 让 view-leave 可真 abort,避免 6 连接池占满 + 切页 carryover)
...
function _prefetchOne(code) {
  ...
  const ctrl = new AbortController();
  _prefetchInflight.set(code, ctrl);
  // R-opt-2026-07-19: 改用 /core (轻量,SW 5min 缓存) — 旧端点已废弃
  // R-fix-2026-08-01: 加 signal 让 view-leave 可真 abort
  fetch(`/api/stock/${code}/core`, { priority: 'low', signal: ctrl.signal })
...
function _prefetchAbortAll() {
  for (const c of _prefetchInflight.values()) {
    try { c.abort(); } catch (e) {}
  }
  _prefetchInflight.clear();
  _prefetchQueue = [];
  if (_prefetchHoverT) { clearTimeout(_prefetchHoverT); _prefetchHoverT = null; }
}
...
(function registerSW() {
  ...
  // R-mobile-refresh: 新 SW 接管 → 自动 reload,避免 iOS Safari 卡在旧 cache
  navigator.serviceWorker.addEventListener('controllerchange', () => {
    // controller 切换触发 reload,带 bustQuery 防再次走 cache
  ...
  // 延迟到 idle,不影响首屏
  const _do = (update) => navigator.serviceWorker.register(__SW_URL__, { scope: '/', updateViaCache: 'none' })
    .then((reg) => {
      // R34: 每次加载检查 SW 更新 (浏览器默认每 24h 才查一次)
      if (update) reg.update().catch(() => {});
      // 检测到新 SW → 通知它立即激活
      if (reg.waiting && navigator.serviceWorker.controller) {
        reg.waiting.postMessage({ type: 'SKIP_WAITING' });
      }
      // installing 也加快 — 每次加载检查 install 完成
      if (reg.installing) {
  ...
  if ('requestIdleCallback' in window) {
    requestIdleCallback(() => _do(false), { timeout: 4000 });
  } else {
    setTimeout(() => _do(false), 1500);
  }
  // 页面可见时额外检查一次更新(覆盖用户回来时的情况)
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden) _do(true);
  }, { once: true });
})();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('#sidebar [data-jump]').forEach(el => {
...
// R-fix-2026-08-01: 不只清 Set,还 abort 所有 controller,真把 in-flight prefetch fetch 杀掉
  ...
  try {
    ...
    box.innerHTML = results.map(s =>
      `<button class="result-pill" data-code="${escapeHtml(s.code)}" data-name="${escapeHtml(s.name)}">
        ...
        <span class="rp-dxin" data-action="dexin-check:${escapeHtml(s.code)}" data-stop="1" title="得鑫量变术 5 阶段判定">验</span>
      </button>`).join('');
...
// R62: 自适应轮询间隔 — A 股交易时间 9:30-11:30 / 13:00-15:00 10s,其他 60s
function _tradingIntervalMs() {
  const now = new Date();
  // 服务端时区是 Asia/Shanghai (UTC+8),本地浏览器可能不是,所以用 UTC 转换
  const cn = new Date(now.toLocaleString('en-US', { timeZone: 'Asia/Shanghai' }));
  const day = cn.getDay();  // 0=Sun 6=Sat
  if (day === 0 || day === 6) return 60_000;
  const m = cn.getHours() * 60 + cn.getMinutes();
  if (m >= 9 * 60 + 30 && m < 11 * 60 + 30) return 10_000;
  if (m >= 13 * 60 && m < 15 * 60) return 10_000;
  return 60_000;
}
...
function _startStockPoll(code) {
  ...
  // R-fix-2026-07-17: 首轮 poll 从 3s 推到 10s — loadStockDetail 已经刚拉过实时主数据,
  // 3s 后再重拉完全重复 (主 API 内已 _startStockPoll + 旧轮询 + loadStockDetail 各 1 次 = 浪费)
  // R62: 自适应间隔 — 交易时段 10s,非交易 60s,周末 60s
  function _scheduleNext(delay) {
    _stockPollTimer = setTimeout(() => {
      if (!document.hidden && currentStockCode) _pollStockRealtime(currentStockCode);
...
async function _pollStockRealtime(code) {
  ...
  // R2: 节流:若上一次还没回(<8s)就跳过,避免重叠
  if (_stockPollLastTs && Date.now() - _stockPollLastTs < 8000) return;
  _stockPollLastTs = Date.now();
  // R1: 用全局 abort controller — 切股时本次请求立即取消,响应到达也丢弃
  const ctrl = _newStockFetchCtrl();
  try {
    // 2026-07-18 perf: 轮询不强制 _fresh — Redis 5s 缓存足够新鲜,免得每次绕缓存打 3s+
    // 历史快照模式下还是带 date,避免 poll 把历史切回实时
    const dateInput = $('#stock-date');
    const curDate = dateInput?.value || '';
    const qs = curDate ? `?date=${encodeURIComponent(curDate)}` : '';
    const data = await api(`/api/stock/${code}${qs}`, { signal: ctrl.signal });
    if (!data || code !== currentStockCode) return;
    if (ctrl.signal.aborted) return;  // R1: 已被 abort,丢弃
    if (data.is_historical) {
      // 历史快照模式:全量重渲染 (patch 没考虑历史快照字段)
      renderStockDetail(code, data);
    } else {
      // R33: 价格/涨跌/资金全相同时完全跳过 patch,避免 1Hz SSE 触发无意义 DOM 写
      const sig = (data.quote?.price ?? '') + '|' + (data.quote?.change_pct ?? '') + '|' + (data.fund_flow?.today?.main_net ?? '') + '|' + (data.quote?._fetch_time ?? '');
      if (sig !== _patchStockRealtime._lastSig) {
        _patchStockRealtime._lastSig = sig;
        _patchStockRealtime(code, data);
      }
    }
    // 2026-07-18 R3: 只在价格/资金净流真的变了才写 sessionStorage,
    // 10s 一次的轮询每秒都 JSON.stringify 36KB kline 数组,卡主线程
    const cacheSig = (data.quote?.price ?? '') + '|' + (data.quote?.change_pct ?? '') + '|' + (data.fund_flow?.today?.main_net ?? '');
    if (cacheSig !== _pollStockRealtime._lastCacheSig) {
      _pollStockRealtime._lastCacheSig = cacheSig;
...
// R7: RAF-batched price patch — 攒多个 DOM 写操作到同一帧
var _patchPending = null;

// R1: 全局 stock fetch abort controller — 切股/切 view 时取消旧请求,避免延迟到达的响应污染新页面
var _stockFetchCtrl = null;
function _abortStockFetches() {
  if (_stockFetchCtrl) {
    try { _stockFetchCtrl.abort(); } catch {}
    _stockFetchCtrl = null;
  }
}
function _newStockFetchCtrl() {
  _abortStockFetches();
  _stockFetchCtrl = new AbortController();
  return _stockFetchCtrl;
}

function _schedulePatch(fn) {
  _patchPending = fn;
  if (!_patchPending._raf) {
    _patchPending._raf = true;
    const captured = fn;        // R-fix-2026-07-18: 闭包捕获避免 outer var 竞态
    requestAnimationFrame(() => {
      try { captured(); } catch (e) { console.warn('[patch]', e); }
      // 只重置如果还是同一个 fn (避免覆盖已被新调用 set 的 _patchPending)
      if (_patchPending === captured) _patchPending = null;
    });
  }
}

function _patchStockRealtime(code, data) {
  // R7: stale code guard — SSE/RAF 延迟到达时若已切股,跳过渲染防覆盖新股价
  if (code && window._currentStockCode && code !== window._currentStockCode) return;
  const q = data.quote || {};
  const today = (data.fund_flow || {}).today || {};

  // R25: 更新数据新鲜度 — 保留日期 + 追加实时时间 (原来直接覆盖丢失日期)
  if (q._fetch_time) {
    const subEl = $('#stock-sub');
    if (subEl) {
      const existing = subEl.dataset.dateLabel || '';
      subEl.textContent = existing
        ? `${existing} · 实时 ${q._fetch_time}`
        : `实时 ${q._fetch_time}`;
      // q-time 单独显示时间,方便眼扫
      const qTime = $('#q-time');
      if (qTime) qTime.textContent = `实时 ${q._fetch_time}`;
    }
  }
  ...
  if (price > 0) {
    ...
    if (priceEl) {
      ...
      // 2026-07-18 perf: SSE 1Hz tick 时常只有 0.01 元抖动,触发 RAF 350ms 重画是浪费;
      // 阈值改成 > 0.1% refVal (茅台 1.2 元 / 比亚迪 0.1 元) 才动,否则直接写文本
      const animThresh = Math.max(prev * 0.001, 0.05);
      if (prevPrice > 0 && Math.abs(price - prevPrice) > animThresh) {
...
async function loadStockDetail(code, date) {
  ...
  // 2026-07-18 perf: 不再强制 ?_fresh=1 — Redis 5s 缓存足够新鲜,免得每次绕缓存 3s+
  // 用户手动点 refresh 时,业务方单独传 ?fresh=1 触发 bypass
  const qs = dateParam
  ...
  if (cached) {
    if (currentStockCode !== code) return;  // 用户已切股,放弃过期 render
  ...
  try {
    ...
    // R-perf-018: 拉数据期间用户可能已切走,过期结果不再 render / poll
...
function renderSeatBreakdown(d) {
  ...
  tagsEl.innerHTML = (Array.isArray(d.tags) ? d.tags : []).map(t => {
  ...
  risksEl.innerHTML = (Array.isArray(d.risks) ? d.risks : []).map(r =>
...
function drawKlineChart() {
  ...
  // R1 (kline-1000rounds): 左/右 gutter 56→64 让 y 轴标签 (如 1234.56 元) 不被切;
  //   top 12→16 给主图头部一点呼吸空间,避免 MA endLabel 与图顶齐平。
  const hasSub = !!subIndicator;
  const GUTTER = 64;
  const grids = [
    { left: GUTTER, right: GUTTER, top: 16, height: hasSub ? '58%' : '70%' },
    { left: GUTTER, right: GUTTER, top: hasSub ? '72%' : '74%', height: hasSub ? '14%' : '20%' },
  ];
  if (hasSub) grids.push({ left: GUTTER, right: GUTTER, top: '88%', height: '10%' });
  ...
  const yAxes = [
    { scale: true, gridIndex: 0,
      splitLine: { lineStyle: { color: GRID } },
      axisLabel: { color: INK2, fontSize: 10, formatter: v => {
        // R3 (kline-1000rounds): 千分位 + 智能 2 位小数 (1234.567 → 1,234.57)
        const n = +v;
  ...
  // ── Series ──
  const series = [];
  // K线（主图）— R4 (kline-1000rounds): 最后 1 根高亮 + markLine 标示当前价
  // R5: label 加涨跌额/涨跌幅 (与最近 1 个交易日对比),色跟随涨跌
  // R6: label 加 ▲/▼ 方向箭头 + 实色背景,涨跌一眼可辨
  const lastIdx = dates.length - 1;
  const lastClose = closes[lastIdx];
  const prevClose = lastIdx > 0 ? closes[lastIdx - 1] : kline[0].open;
  const _chg = lastClose - prevClose;
  const _chgPct = prevClose ? (_chg / prevClose) * 100 : 0;
  const _chgSign = _chg >= 0 ? '+' : '';
  const _chgColor = _chg >= 0 ? UP : DOWN;
  const _arrow = _chg >= 0 ? '▲' : '▼';
  const _priceStr = lastClose.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const _chgStr = `${_chgSign}${_chg.toFixed(2)} (${_chgSign}${_chgPct.toFixed(2)}%)`;
  series.push({
    ...
    markLine: {
      ...
      label: {
        ...
        borderColor: _chgColor,
        borderWidth: 0,
        borderRadius: 3,
        padding: [4, 7],
        formatter: () => `${_arrow} ${_priceStr}\n${_chgStr}`,
      },
      lineStyle: {
        color: _chgColor,
        width: 1,
  ...
  if (ind.ma) {
    ...
    series.push({ name: 'MA20', type: 'line', data: ma20, smooth: true, lineStyle: { color: ACCENT,  width: 1.2 }, symbol: 'none', connectNulls: true,
      endLabel: { show: true, formatter: 'MA20', color: ACCENT, fontSize: 10, fontWeight: 600 } });
    series.push({ name: 'MA60', type: 'line', data: ma60, smooth: true, lineStyle: { color: 'var(--accent-3)', width: 1.2 }, symbol: 'none', connectNulls: true,
      endLabel: { show: true, formatter: 'MA60', color: 'var(--accent-3)', fontSize: 10, fontWeight: 600 } });
  }
  // BOLL 叠加（主图）— R2: BOLL 中轨 endLabel,上下轨不加(避免拥挤)
  if (boll) {
    ...
    series.push({ name: 'BOLL中', type: 'line', data: boll.mid,   smooth: true, lineStyle: { color: 'var(--accent)', width: 0.8, opacity: 0.7 }, symbol: 'none', connectNulls: true,
      endLabel: { show: true, formatter: 'BOLL', color: 'var(--accent)', fontSize: 10 } });
...
// STOCK · 任意日分时回看
// ────────────────────────────────────────────
var INTRADAY_CACHE_MAX = 200;   // B-15: LRU 上限, 防止切 N 只股 × M 日期 累积
var intraDayCache = new Map();  // date -> data
var intraDayLoading = null;

// B-15: LRU 写入 — 超限淘汰最早插入
...
async function loadIntraDay(code, dateStr) {
  ...
  try {
    ...
    _intraDayCacheSet(dateStr, merged);
...
function drawIntraDayChart(code, date, ticks, openRef, prevClose, limitUp) {
  ...
  chart.setOption({
    ...
    xAxis: [
      { type: 'category', data: times, gridIndex: 0, axisLine: { lineStyle: { color: CHART_LINE } },
        axisLabel: { color: INK2, fontSize: 10 }, splitLine: { show: false } },
      { type: 'category', data: times, gridIndex: 1, axisLine: { lineStyle: { color: CHART_LINE } },
...
// STOCK 内部 tab
// ────────────────────────────────────────────
// 2026-07-16: 这里原本有重复的 .tab[data-tab] click handler (与 view-stock.js 完全一样)，
// 现在 view-stock.js 维护单一源 (含完整的 chart draw / resize / lazy-load 逻辑)。
...
async function renderLawsOnce() {
  ...
  const cats = Array.isArray(lawsData.categories) ? lawsData.categories : [];
...
function renderDragons(data) {
  ...
  if (main.length === 0) {
    ...
    $$('#dragons-mainline .sector-link').forEach(a => {
      a.onclick = e => {
        e.preventDefault();
        // 板块名 → 跳全 A 风向,按该板块过滤 (l1=L1主题名, l2=板块名)
  ...
  // 昨日涨停 — 跟今日全场对比,显示「连板变化」(今日连板 - 昨日连板)
  const yestList = (d.yesterday_all || []).slice();
  $('#dragons-yesterday-date').textContent = d.yesterday_date ? `(${d.yesterday_date.slice(4,6)}/${d.yesterday_date.slice(6,8)})` : '';
  const todayStreakByCode = {};
  for (const s of (d.all || [])) {
    if (s && s.code) todayStreakByCode[s.code] = s.streak || 1;
  }
  function _yestSortKey(z) {
    const today = todayStreakByCode[z.code];
    const y = z.streak || 1;
    if (today == null) return -100 + y;
    if (today > y) return 1000 + today;
    if (today === y) return 500 + y;
    return 200 + today;
  }
  yestList.sort((a, b) => _yestSortKey(b) - _yestSortKey(a));
  $('#dragons-yesterday-count').textContent = yestList.length;
  const yBody = $('#dragons-yesterday-table tbody');
  if (yestList.length === 0) {
    yBody.innerHTML = '<tr><td colspan="12" class="empty">无昨日涨停数据</td></tr>';
  } else {
    yBody.innerHTML = yestList.map((z, i) => {
      const sealTxt = z.seal_ratio_pct != null ? `${z.seal_ratio_pct.toFixed(1)}%` : '—';
      const yStreak = z.streak || 1;
      const tStreak = todayStreakByCode[z.code];
      let deltaCell;
      if (tStreak == null) {
        deltaCell = `<span class="streak-badge streak-dn" title="今日未封板">断板</span>`;
      } else {
        const diff = tStreak - yStreak;
        if (diff > 0)         deltaCell = `<span class="streak-badge streak-new" title="今日继续封板, 连板提升">↑${diff}</span>`;
        else if (diff === 0)  deltaCell = `<span class="streak-badge streak-same" title="今日平板">—</span>`;
        else                  deltaCell = `<span class="streak-badge streak-dn" title="今日连板降低">↓${Math.abs(diff)}</span>`;
      }
      // v234b (2026-07-28): 今日涨幅 + 今日涨停 chip
      const yChange = z.change_pct != null ? Number(z.change_pct) : null;
      const isZt = !!z.is_zt_today;
      const chgTxt = yChange != null
        ? `<b style="color:${yChange >= 0 ? 'var(--up)' : 'var(--down)'}">${yChange >= 0 ? '+' : ''}${yChange.toFixed(2)}%</b>`
        : '<span class="dim">—</span>';
      const chgType = z.change_type || '';
      const ztBadge = isZt
        ? `<span class="streak-badge streak-new" title="今日继续封板">涨停</span>`
        : (yChange != null && yChange <= -7
            ? `<span class="streak-badge streak-dn" title="今日大跌 ≥7%">大面</span>`
            : (chgType ? `<span class="streak-badge streak-same" title="${escapeHtml(chgType)}">${escapeHtml(chgType)}</span>` : '<span class="dim">—</span>'));
      return `<tr>
        <td>${i + 1}</td>
        ...
        <td>${escapeHtml(sealTxt)}</td>
        <td>${escapeHtml(String(z.score_total != null ? z.score_total.toFixed(1) : '—'))}</td>
        <td>${chgTxt}</td>
        <td>${ztBadge}</td>
      </tr>`;
    }).join('');
  }
  // 备注: 实时统计 昨日→今日 的 升板 / 平板 / 断板 + 今日涨停 chip
  if (yestList.length > 0) {
    let upgraded = 0, broken = 0, held = 0, zt_today = 0, big_down = 0;
    for (const z of yestList) {
      const yS = z.streak || 1;
      const tS = todayStreakByCode[z.code];
      if (tS == null) broken++;
      else if (tS > yS) upgraded++;
      else if (tS === yS) held++;
      if (z.is_zt_today) zt_today++;
      if (z.change_pct != null && z.change_pct <= -7) big_down++;
    }
    const note = `昨日 <b>${yestList.length}</b> 只涨停 · 今日继续封板 <b style="color:var(--up)">${zt_today}</b> 只 (升板 ${upgraded} / 平板 ${held}) · 断板 <b style="color:var(--down)">${broken}</b> 只 (含大面 ${big_down})`;
...
async function loadDragons(refresh = false) {
  ...
  // R31: 首次加载显示 skeleton
  if (!_dragonsLoaded) {
    const skel = (n) => '<div class="skeleton skel-line" style="width:'+n+'%;height:16px;margin:8px 0"></div>';
    $('#dragons-sentiment-label').innerHTML = skel(40);
    $('#dragons-zt-count').textContent = '…'; $('#dragons-max-streak').textContent = '…板';
    $('#dragons-mainline').innerHTML = Array.from({length:3},()=>'<div style="padding:12px">'+skel(60)+skel(30)+'</div>').join('');
    $('#dragons-top10').innerHTML = Array.from({length:3},()=>'<div class="dragon-card">'+skel(50)+skel(70)+skel(90)+'</div>').join('');
  }
  try {
    ...
    toast('龙头加载失败');
    if (!_dragonsLoaded) {
      $('#dragons-top10').innerHTML = '<div class="empty-state error"><span class="empty-icon">⚠</span><span class="empty-msg">加载失败: '+escapeHtml(e.message)+'</span><button class="btn-mini primary" onclick="loadDragons(false)">重试</button></div>';
    }
  } finally {
...
showView = function(name, ctx) {
  const prev = _currentViewName;
  // R-fix-2026-08-01: 切页时统一 abort 全部 inflight + SSE,跟各 view 自管
  // _VIEW_LEAVE_HOOKS 互补 (hook 漏注册时也兜底),避免旧 view 数据污染新 view
  // + HTTP/1.1 6 连接池被旧请求占满 (频繁切页卡死根因之一)
  if (prev && prev !== name) {
    try { _inflightAbortAll(); } catch (e) { console.warn('inflight abort:', e); }
    try { _abortAllSSE(); } catch (e) { console.warn('sse abort:', e); }
  }
...
$('#theme-toggle')?.addEventListener('click', () => {
  ...
  refreshThemeColors();
  refreshChartColors();
  // 更新 ECharts 实例颜色 (增量 setOption,避免全量 re-render 卡顿)
  var _tl = CHART_LINE, _tt = CHART_TOOLTIP_BG, _tb = CHART_TOOLTIP_BORDER;
  Object.values(echartsCharts).forEach(c => {
    if (!c) return;
    try { c.resize(); } catch {}
    try {
      c.setOption({
        xAxis: { axisLine: { lineStyle: { color: _tl } },
                axisLabel: { color: INK2 } },
        yAxis: { splitLine: { lineStyle: { color: GRID } },
                axisLabel: { color: INK2 },
                axisLine: { lineStyle: { color: _tl } } },
        tooltip: { backgroundColor: _tt, borderColor: _tb,
                   textStyle: { color: INK } },
      });
    } catch {}
  });
...
// R-3-2026-07-18: URL ?theme=light/dark 参数 — 用于截图/分享指定主题 (不带 localStorage,刷新后失效)
...
// resize + refreshTicker 启动 → view-other.js:671-677 统一注册 (避免双 setInterval 拖双倍流量)
...
// refreshTunnel + setInterval(refreshTunnel, 10s) → view-other.js 统一注册 (避免双轮询)
...
// SECTOR DETAIL · 板块详情页 (从 dragons 跳转过来时 ctx = {name: '板块名'})
async function renderSectorDetail(ctx) {
  const view = $('.view-sector');
  if (!view) return;
  // ctx 兼容四种: {name:'板块名'} (dragons 链接) / string '板块名' (deep-link) /
  //              {arg:'板块名'} (老 audit 工具) / {push:false} (路由过来)
  let name = '';
  if (ctx) {
    if (typeof ctx === 'string') name = ctx;
    else if (ctx.name) name = ctx.name;
    else if (ctx.arg) name = ctx.arg;
  }
  const status = $('#sector-status');
  if (status) { status.textContent = name ? `加载 ${name} …` : '未指定板块'; status.style.color = INK2; }
  // 没名字就只显示空状态,不再乱发请求
  if (!name) {
    $('#sector-name').textContent = '—';
    $('#sector-change').textContent = '—';
    $('#sector-flow').textContent = '—';
    $('#sector-zt-count').textContent = '—';
    $('#sector-roles').innerHTML = '<p class="empty">请从龙头战法页面点击板块名进入</p>';
    $('#sector-zt-table tbody').innerHTML = '<tr><td colspan="6" class="empty">—</td></tr>';
    $('#sector-5d-table tbody').innerHTML = '<tr><td colspan="5" class="empty">—</td></tr>';
    return;
  }
  try {
    const data = await api(`/api/sector/${encodeURIComponent(name)}`);
    if (!data || !data.info) {
      if (status) { status.textContent = `未匹配板块: ${name}`; status.style.color = DOWN; }
      return;
    }
    const info = data.info;
    $('#sector-name').textContent = info.name || name;
    $('#sector-change').textContent = `${info.change_pct >= 0 ? '+' : ''}${(info.change_pct || 0).toFixed(2)}%`;
    $('#sector-change').style.color = info.change_pct >= 0 ? UP : DOWN;
    $('#sector-flow').textContent = `${(info.net_inflow_yi || 0).toFixed(2)} 亿`;
    $('#sector-zt-count').textContent = String(info.zt_count || 0);
    // 角色分布
    const rd = data.role_dist || {main: 0, second: 0, noise: 0};
    $('#sector-roles').innerHTML = `
      <div class="role-chip role-main">
        <span class="role-num">${rd.main}</span>
        <span class="role-label">龙头</span>
      </div>
      <div class="role-chip role-second">
        <span class="role-num">${rd.second}</span>
        <span class="role-label">中军</span>
      </div>
      <div class="role-chip role-noise">
        <span class="role-num">${rd.noise}</span>
        <span class="role-label">杂毛</span>
      </div>`;
    // 今日 zt 列表
    const todayZt = data.today_zt || [];
    $('#sector-zt-rows-count').textContent = String(todayZt.length);
    $('#sector-zt-table tbody').innerHTML = todayZt.length === 0
      ? '<tr><td colspan="6" class="empty">暂无涨停</td></tr>'
      : todayZt.map(z => `<tr>
          <td><a href="#" class="stock-link" data-code="${escapeHtml(z.code)}">${escapeHtml(z.code)}</a></td>
          <td>${escapeHtml(z.name || '')}</td>
          <td>${escapeHtml(String(z.streak || 0))}</td>
          <td>${escapeHtml(z.time || '—')}</td>
          <td>${z.seal_pct != null ? escapeHtml(`${z.seal_pct}%`) : '—'}</td>
          <td>${escapeHtml(z.sub_chain || z.sw || '—')}</td>
        </tr>`).join('');
    // 近 5 日高发
    const hot5 = data.hot_5d || [];
    $('#sector-5d-count').textContent = String(hot5.length);
    $('#sector-5d-table tbody').innerHTML = hot5.length === 0
      ? '<tr><td colspan="5" class="empty">暂无数据</td></tr>'
      : hot5.map(h => `<tr>
          <td><a href="#" class="stock-link" data-code="${escapeHtml(h.code)}">${escapeHtml(h.code)}</a></td>
          <td>${escapeHtml(h.name || '')}</td>
          <td>${escapeHtml(String(h.zt_count || 0))}</td>
          <td>${escapeHtml(String(h.total_streak || 0))}</td>
          <td>${escapeHtml(h.last_date || '—')}</td>
        </tr>`).join('');
    if (status) { status.textContent = `已加载 ${info.name} · ${todayZt.length} 只涨停 · 5d ${hot5.length} 只`; status.style.color = INK3; }
  } catch (e) {
    if (status) { status.textContent = `加载失败: ${e.message}`; status.style.color = DOWN; }
  }
}
  ...
  try {
    ...
    if (news.length) {
      host5.innerHTML = `
        ...
        <p class="caption dim" style="margin:.25rem 0 0">命中 ${news.length} 条（最新在前）</p>`;
    } else {
...
function _reviewRender() {
  ...
  if (!_reviewState.trades.length) {
    tbody.innerHTML = '<tr><td colspan="12" class="dim center">暂无交易 · 上面录入第一笔</td></tr>';
    return;
  }
  ...
  for (const [code, list] of groups) {
    ...
    // 自选状态:通过 star-btn class active 标记
    const inWl = _watchlistItems ? _watchlistItems.some(x => x.code === code) : false;
    ...
    html.push(`
      <tr class="rv-group-hd ${expandable ? 'rv-expandable' : ''}" data-code="${escapeHtml(code)}" data-group="${gid}">
        ...
        <td class="rv-star-cell"><span class="star-btn ${inWl ? 'active' : ''}" data-star-code="${escapeHtml(code)}" data-star-name="${escapeHtml(name)}" title="加自选"></span></td>
      </tr>
    `);
    if (expandable) {
      // 折叠明细:子表显示每一笔独立行
      ...
      html.push(`
        <tr class="rv-child-wrap" data-group="${gid}" hidden>
          <td colspan="12" class="rv-child-cell">
            <table class="data-table review-table review-table-child">
              <thead>
                <tr>
                  <th class="rv-sticky-1">子笔明细</th>
                  <th>方向</th>
                  <th>日期</th>
                  <th>价格</th>
                  <th>时间</th>
                  <th>股数</th>
                  <th>今日盈亏</th>
                  <th class="rv-hide-tablet">累计盈亏</th>
                  <th class="rv-hide-tablet">累计盈亏比</th>
                  <th class="rv-hide-tablet">铁律错在哪</th>
                  <th class="rv-hide-mobile">操作</th>
                </tr>
              </thead>
  ...
  // ── 一键折叠/展开全部 ──
  const toggleAllBtn = $('#review-toggle-all');
  if (toggleAllBtn && !toggleAllBtn._bound) {
    toggleAllBtn._bound = true;
    toggleAllBtn.addEventListener('click', () => {
      const allWraps = tbody.querySelectorAll('tr.rv-child-wrap[data-group]');
      if (!allWraps.length) return;
      // 判断当前状态: 有任何一个是展开的 → 全部折叠; 否则全部展开
      const anyOpen = Array.from(allWraps).some(w => !w.hidden);
      allWraps.forEach(wrap => {
        const gid = wrap.dataset.group;
        const btn = tbody.querySelector(`.rv-expand-btn[data-toggle="${gid}"]`);
        if (!btn) return;
        wrap.hidden = anyOpen;  // 要折叠 → hidden=true
        btn.textContent = anyOpen ? '▶' : '▼';
        btn.classList.toggle('open', !anyOpen);
        btn.setAttribute('aria-expanded', String(!anyOpen));
      });
      toggleAllBtn.textContent = anyOpen ? '📂 全部展开' : '📂 全部折叠';
    });
  }

  // ── 快速选股：按代码/名称过滤 ──
  const qf = $('#review-quick-filter');
  if (qf && !qf._bound) {
    qf._bound = true;
    qf._qTimer = null;
    qf._origHidden = new Map();  // 存档各组过滤前的 hidden 状态
    qf.addEventListener('input', () => {
      clearTimeout(qf._qTimer);
      qf._qTimer = setTimeout(() => {
        const q = qf.value.trim().toLowerCase();
        const hasFilter = !!q;
        tbody.querySelectorAll('tr.rv-group-hd[data-code]').forEach(tr => {
          const code = (tr.dataset.code || '').toLowerCase();
          const nameEl = tr.querySelector('.np-name');
          const name = (nameEl?.textContent || '').trim().toLowerCase();
          const match = !hasFilter || code.includes(q) || name.includes(q);
          const gid = tr.dataset.group;
          const wrap = gid ? tbody.querySelector(`tr.rv-child-wrap[data-group="${gid}"]`) : null;
          if (hasFilter) {
            // 过滤模式: 不匹配 → 整组隐藏; 匹配 → 只显主行,明细也隐藏
            tr.hidden = !match;
            if (wrap) { wrap.hidden = true; wrap.dataset.filtered = '1'; }
          } else {
            // 清除过滤: 恢复主行,子行恢复默认展开状态
            tr.hidden = false;
            if (wrap) {
              const wasCollapsed = qf._origHidden.has(gid) && qf._origHidden.get(gid);
              wrap.hidden = wasCollapsed;
              delete wrap.dataset.filtered;
              // 同步展开按钮
              const btn = tbody.querySelector(`.rv-expand-btn[data-toggle="${gid}"]`);
              if (btn) {
                btn.textContent = wasCollapsed ? '▶' : '▼';
                btn.classList.toggle('open', !wasCollapsed);
                btn.setAttribute('aria-expanded', String(!wasCollapsed));
              }
            }
          }
          // 首次进入过滤时记下原始隐藏状态
          if (hasFilter && gid && wrap && !qf._origHidden.has(gid)) {
            qf._origHidden.set(gid, !!wrap.hidden);
          }
        });
      }, 150);
    });
  ...
  // ── 表头排序: 单击 th.sortable 切换升降序 ──
  const sortState = _reviewState._sort || { key: '', dir: 'asc' };
  _reviewState._sort = sortState;
  ...
  if (thead) {
    thead.querySelectorAll('th.sortable').forEach(th => {
      ...
      if (!key) return;
      // 标记当前排序列
      th.classList.toggle('sort-active', sortState.key === key);
      ...
      th.addEventListener('click', () => {
        ...
        _reviewSortAndRender(sortState.key, sortState.dir);
      });
    });
  }

  // ── 自选 ⭐ star-btn 委托 ──
  const reviewTable = tbody.closest('table');
  if (reviewTable && !reviewTable._starDelegate) {
    reviewTable._starDelegate = true;
    reviewTable.addEventListener('click', async (e) => {
      const star = e.target.closest('.star-btn[data-star-code]');
      if (!star) return;
      e.stopPropagation();
      ...
      if (isActive) {
        try {
          ...
          if (env.ok || env.data?.removed) {
            ...
            toast('✗ 删除失败: ' + (env.error || ''), 'error', 3000);
          }
        } catch (err) {
          toast('✗ 删除失败: ' + err.message, 'error', 3000);
        }
      } else {
        try {
          const resp = await fetch('/api/watchlist', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ code, name, tag: '复盘' }),
          });
          const env = await resp.json();
          if (env.ok) {
            ...
            toast('✗ 加入失败: ' + (env.error || ''), 'error', 3000);
          }
        } catch (err) {
          toast('✗ 加入失败: ' + err.message, 'error', 3000);
        }
      }
    });
  }
...
/**
 * 按 key 排序 review 表,重渲 tbody 行顺序 (不重新抓数据)
 */
function _reviewSortAndRender(key, dir) {
  const tbody = $('#review-tbody');
  if (!tbody) return;
  const rows = Array.from(tbody.querySelectorAll('tr.rv-group-hd'));
  if (!rows.length) return;
  // 每行附带对应的子行 wrap
  ...
  pairs.sort((a, b) => {
    const ta = a.tr, tb = b.tr;
    const getVal = (tr, k) => {
      if (k === 'name') return (tr.querySelector('.np-name')?.textContent || tr.dataset.code || '').trim();
      if (k === 'direction') return (tr.querySelector('.group-summary-cell')?.textContent || '').trim();
      if (k === 'date') return tr.querySelector('td:nth-child(3)')?.textContent?.trim() || '';
      if (k === 'price') return parseFloat(tr.querySelector('td:nth-child(4)')?.textContent?.replace(/[,，]/g, '') || '0');
      if (k === 'time') return tr.querySelector('td:nth-child(5)')?.textContent?.trim() || '';
      if (k === 'shares') {
        const txt = tr.querySelector('td:nth-child(6)')?.textContent?.replace(/[^0-9.]/g, '') || '0';
        return parseInt(txt) || 0;
      }
      if (k === 'today') return parseFloat(tr.querySelector('td:nth-child(7)')?.textContent?.replace(/[^0-9.\-]/g, '') || '0');
      if (k === 'cum') return parseFloat(tr.querySelector('td:nth-child(8)')?.textContent?.replace(/[^0-9.\-]/g, '') || '0');
      if (k === 'cumPct') return parseFloat(tr.querySelector('td:nth-child(9)')?.textContent?.replace(/[^0-9.\-]/g, '') || '0');
      return '';
    };
    let va = getVal(ta, key), vb = getVal(tb, key);
    let cmp = 0;
    if (typeof va === 'string') {
  ...
  pairs.forEach(p => {
    fragment.appendChild(p.tr);
    if (p.wrap) fragment.appendChild(p.wrap);
  });
  ...
  if (thead) {
    thead.querySelectorAll('th.sortable').forEach(th => {
      ...
      const arrow = th.querySelector('.sort-arrow');
      if (arrow) arrow.textContent = th.dataset.sort === key ? (dir === 'asc' ? ' ▲' : ' ▼') : '';
    });
  }
  // 更新筛选输入框状态 (保留)
  const qf = $('#review-quick-filter');
  if (qf && qf.value.trim()) qf.dispatchEvent(new Event('input'));
}
  ...
  if (codeInput && nameInput) {
    ...
    function _showSuggest(items) {
      ...
      _searchBox.style.cssText = `
        position: absolute; background: var(--glass-bg-strong);
      ...
      items.slice(0, 10).forEach(item => {
        ...
        row.className = 'review-suggest-row';
        row.innerHTML = `<code class="review-suggest-code">${item.code}</code> <span class="review-suggest-name">${escapeHtml(item.name || '')}</span>`;
...
function _reviewOnViewEnter() {
  if (document.querySelector('.view-review:not([hidden])')) {
    ...
    // R15: 默认不自动 AI 复盘。用户如需批量复盘,可点页面上的"一键复盘全部"按钮,或逐笔点 AI 复盘。
  }
}
...
function _stockOnViewLeave() {
  ...
  // R3: 全局 abort controller 也清掉 — 切到其他 view 时立即取消所有飞行中的 stock 请求
  _abortStockFetches();
  // R8b: 清空 view-scoped timers (streak prefetch chain 等)
  if (typeof _clearViewTimers === 'function') _clearViewTimers('stock');
  // R13: 清掉 inflight dedup promise (防止下次进 view 拿到旧 promise)
  if (typeof _klineInflight !== 'undefined') { _klineInflight = null; _klineInflightKey = ''; }
  if (typeof _stockDetailInflight !== 'undefined') { _stockDetailInflight = null; _stockDetailInflightKey = ''; }
  // R13: 清掉 _stockAuxCache (避免旧 code 的 sector/lu_ctx 残留内存)
  if (typeof _stockAuxCache === 'object') {
    _stockAuxCache.code = null;
  ...
  // R14: dispose ECharts 实例释放 GPU + canvas 内存 (个股页 4 个图常驻 ~50-100MB)
  if (typeof echartsCharts === 'object') {
    for (const k of Object.keys(echartsCharts)) {
      try { if (echartsCharts[k]) { echartsCharts[k].dispose(); echartsCharts[k] = null; } } catch {}
    }
  }
  // R16: 取消挂着的 animateNumber RAF — 切股时旧动画不再影响新价格
  if (typeof _cancelAnimateNumbers === 'function') _cancelAnimateNumbers();
  // R19: 清掉 pending 图表数据 — 切股时旧 flow/kline 数据不应再被绘制
  if (typeof _pendingFlowData !== 'undefined') _pendingFlowData = null;
  if (typeof _klineDataReady !== 'undefined') _klineDataReady = false;
  // R10 (Batch 1): 输出 100 轮缓存命中率 (Playwright 抓 console 验证 P95)
  if (typeof _cacheHits === 'object' && _cacheHits) {
    const total = (_cacheHits.mem|0) + (_cacheHits.ss|0) + (_cacheHits.network|0);
    if (total > 0) {
      console.log('[cache-hits]', JSON.stringify(_cacheHits),
                  `hit_rate=${(((_cacheHits.mem + _cacheHits.ss) / total) * 100).toFixed(1)}%`);
    }
  }
}
_registerViewLeave('stock', _stockOnViewLeave);

// R8b: view-scoped timer registry — 离开 view 自动 clearTimeout 全部 timer
// 用法: setViewTimer('stock', () => {...}, 250)  注册
//       _clearViewTimers('stock')                清空(离开 view 时调用)
var _viewTimers = {};
function setViewTimer(view, fn, ms) {
  const t = setTimeout(() => {
    _viewTimers[view] = (_viewTimers[view] || []).filter(x => x !== t);
    try { fn(); } catch (e) { console.debug('[viewTimer]', view, e.message); }
  }, ms);
  _viewTimers[view] = (_viewTimers[view] || []);
  _viewTimers[view].push(t);
  return t;
}
function _clearViewTimers(view) {
  const list = _viewTimers[view] || [];
  for (const t of list) { try { clearTimeout(t); } catch {} }
  _viewTimers[view] = [];
}
...
function _watchlistRowHtml(it) {
  ...
  if (!ai) {
    ...
    // B-18: verdict 转 string 限制长度,避免 CSS 不命中静默丢样式
    const v = String(ai.verdict || '-').slice(0, 8);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      ...
      if (j.ok && j.data && j.data.ai) {
        ...
        // B-18: verdict 转 string 限长度 + Number 化 conviction
        const vTxt = String(ai.verdict || '-').slice(0, 8);
        const vCls = ({ '买': 'buy', '观望': 'wait', '回避': 'avoid' })[vTxt] || 'na';
        const conv = Number(ai.conviction) || 0;
        item.innerHTML = `<b>${escapeHtml(it.code)}</b> ${escapeHtml(it.name)} — <span class="verdict-pill v-${vCls}">${escapeHtml(vTxt)} ${conv}/100</span>`;
...
function _initTableOverflowHints() {
  ...
  const update = (wrap) => {
    ...
    wrap.classList.toggle('scrollable', has);
  };
...
// R20: visualViewport 键盘避让 — 只在输入框聚焦时把 tabbar 推到键盘上方
// iOS Safari 滚动时地址栏伸缩也会触发 visualViewport resize，
// 必须用 document.activeElement 区分真键盘弹起 vs 地址栏变化。
if ('visualViewport' in window) {
  let _stableVHeight = visualViewport.height;
  ...
  visualViewport.addEventListener('resize', () => {
    const curH = visualViewport.height;
    const diff = _stableVHeight - curH;

    if (_isInputFocused() && diff > 60) {
      _keyboardOpen = true;
      const tb = document.querySelector('.tabbar');
      if (tb) tb.style.transform = `translateY(${-diff}px)`;
    } else if (_keyboardOpen && diff < 30) {
      _keyboardOpen = false;
      const tb = document.querySelector('.tabbar');
      if (tb) tb.style.transform = '';
      _stableVHeight = curH;
    } else if (!_keyboardOpen) {
      // 无人聚焦时持续校准 stable 基准（地址栏伸缩等）
      _stableVHeight = curH;
    }
  });
  ...
  document.addEventListener('focusout', () => {
    if (_keyboardOpen && !_isInputFocused()) {
      _keyboardOpen = false;
      const tb = document.querySelector('.tabbar');
      if (tb) tb.style.transform = '';
      // 延迟校准，等键盘动画结束
      setTimeout(() => { _stableVHeight = visualViewport.height; }, 350);
    }
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $$('#kline-indicators .kt-chip').forEach(btn => {
    btn.addEventListener('click', () => {
      toggleKlineIndicator(btn.dataset.ind);
      _saveKlinePrefs();
    });
  });
});
...
(function() {
  ...
  // === 4. computePageSize =================================================
  // R5-perf (2026-07-15): 结果缓存 + 移动端上限 30 行
  var _lastPageSize = 0, _lastPSCheck = 0;
  function computePageSize() {
    const now = Date.now();
    if (_lastPSCheck && now - _lastPSCheck < 300) return _lastPageSize;
    _lastPSCheck = now;
    const scroll = $('#as-table-scroll');
    if (!scroll) { _lastPageSize = state.pageSize || 30; return _lastPageSize; }
    ...
    let ps = Math.max(15, Math.ceil(visible * 1.5));
    // 窄屏移动端上限 30,避免首屏加载过多行
    if (window.innerWidth < 768) ps = Math.min(ps, 30);
    _lastPageSize = ps;
    return ps;
  }
  ...
  async function loadFilters() {
    try {
      ...
      // 级联映射: clusterName → sw_set,  chainName → {sw, l4}
      state._clusterSwMap = {};
      (data.clusters || []).forEach(c => { state._clusterSwMap[c.name] = c.sw_set || []; });
      state._chainSwMap = {};
      (data.chains || []).forEach(c => { state._chainSwMap[c.name] = { sw: c.sw || '', l4: c.l4 || [] }; });
  ...
  function setSelectOptions(id, items, countMap) {
    const sel = $('#' + id);
    if (!sel) return;
    const counts = countMap || {};
    sel.innerHTML = items.map(x => {
      const c = counts[x] || 0;
      const label = c ? `${escapeHtml(x)} (${c})` : escapeHtml(x);
      return `<option value="${escapeHtml(x)}">${label}</option>`;
    }).join('');
  }
  ...
  // === 5b. 级联筛选 + 计数 (Phase C) ======================================
  function updateCascadeFilters(items) {
    const itemsArr = items || [];
    // 从返回数据统计各层频次
    const l2Count = {}, l3Count = {}, l4Count = {}, domainCount = {};
    itemsArr.forEach(r => {
      const tax = r.taxonomy || {};
      if (tax.l2) l2Count[tax.l2] = (l2Count[tax.l2] || 0) + 1;
      if (tax.l3) l3Count[tax.l3] = (l3Count[tax.l3] || 0) + 1;
      (tax.l4 || []).forEach(x => { l4Count[x] = (l4Count[x] || 0) + 1; });
      (Array.isArray(r.domain) ? r.domain : []).forEach(d => { domainCount[d] = (domainCount[d] || 0) + 1; });
    });

    // L2: 选 L1 后只显示该集群的申万行业
    let l2Options = (state._filterData?.industries || []).slice();
    if (state.l1 && state._clusterSwMap?.[state.l1]) {
      const swSet = new Set(state._clusterSwMap[state.l1]);
      l2Options = l2Options.filter(x => swSet.has(x));
    }
    setSelectOptions('as-l2', l2Options, l2Count);

    // L3: 选 L2 后只显示该行业的产业链
    let l3Options = (state._filterData?.chains || []).map(c => c.name);
    if (state.l2) {
      const l2Set = new Set(state.l2.split(',').filter(Boolean));
      l3Options = l3Options.filter(name => {
        const chain = state._chainSwMap?.[name];
        return chain && l2Set.has(chain.sw);
      });
    }
    setSelectOptions('as-l3', l3Options, l3Count);

    // L4: 选 L3 后只显示该产业链的细分
    let l4Options = (state._filterData?.l4 || []).slice();
    if (state.l3) {
      const l3Set = new Set(state.l3.split(',').filter(Boolean));
      const allowed = new Set();
      l3Set.forEach(l3 => {
        const chain = state._chainSwMap?.[l3];
        (chain?.l4 || []).forEach(x => allowed.add(x));
      });
      if (allowed.size) l4Options = l4Options.filter(x => allowed.has(x));
    }
    setSelectOptions('as-l4', l4Options, l4Count);

    // domain: 始终显示全集 + 计数
    setSelectOptions('as-domain', state._filterData?.domains || [], domainCount);
  }
  ...
  async function loadBoard() {
    ...
    try {
      ...
      if (!data.items || !data.items.length) {
        ...
        tbody.innerHTML = `<tr><td colspan="27" class="empty">
      ...
      renderMeta(data);
      // 2026-07-16: 把 total_candidates / total_universe 传到 KPI strip
      // 之前 host.dataset.cand 永远空,fallback '—' 让用户以为只有 15 只 → 实际是 147 只该类别
      const host = $('#as-kpis');
      if (host) {
        host.dataset.cand    = (data && data.total_candidates) || '';
        host.dataset.universe = (data && data.total_universe) || '';
      }
      state.totalCandidates = (data && data.total_candidates) || state.loadedCount;
      renderAsKpis(data.stats);
      renderTitleSectorBadge(data.stats);
      updateCascadeFilters(data.items);
      updateSentinel();
      // R76-80 (2026-07-17): 数据行渲染后重算横向 overflow + sticky 阴影
      // 初始 _initTableOverflowHints 跑得太早,数据未到,scrollWidth=0; 现在重算
      requestAnimationFrame(() => {
        document.querySelectorAll('.table-wrap').forEach(w => {
          const has = w.scrollWidth > w.clientWidth + 1;
          w.classList.toggle('has-overflow-x', has);
  ...
  async function loadMore() {
    ...
    try {
      ...
      state.hasMore = !!data.has_more;
      renderMeta(data);
      // R-2026-07-16: 滚动追加 stats 不变,只在首屏/cold-cache 路径渲染
      updateSentinel();
    } catch (e) {
  ...
  function renderRows(data, append) {
    ...
    const html = items.map(r => {
      // B-16: 数字字段显式 Number 化 — 后端返字符串/--/null 时安全
      const pct = Number(r.change_pct) || 0;
      const pctCls = pct > 0 ? 'up' : pct < 0 ? 'down' : '';
      // 100-R4: 5 级幅度染色 — 深红≥7 / 淡红≥2 / 灰 / 淡绿≤-2 / 深绿≤-7
      const pctTier =
        pct >= 7  ? 'pct-tier-deep-up'   :
        pct >= 2  ? 'pct-tier-light-up'  :
        pct <= -7 ? 'pct-tier-deep-down' :
        pct <= -2 ? 'pct-tier-light-down' :
                    'pct-tier-zero';
      const pctFl  = pct > 0 ? 'flash-up' : (pct < 0 ? 'flash-down' : '');
      const amt = Number(r.change_amt) || 0;
      const amtCls = amt > 0 ? 'up' : amt < 0 ? 'down' : '';
      const fund = Number(r.main_fund_inflow_wan) || 0;
      ...
      const l1 = tax.l1
        ...
        : '<span class="dim">—</span>';
      // R23 (2026-07-26): 板块角色 — main 龙头 (金) / sub 二线 (银) / spec 杂毛 (灰)
      const role = tax.role || '';
      let roleKey = 'none', roleLabel = '—', roleTitle = '无角色数据';
      if (role === 'main') { roleKey = 'main'; roleLabel = '龙头'; roleTitle = '板块龙头股 (role=main)'; }
      else if (role === 'sub') { roleKey = 'sub'; roleLabel = '二线'; roleTitle = '板块二线股 (role=sub)'; }
      else if (role === 'spec') { roleKey = 'spec'; roleLabel = '杂毛'; roleTitle = '板块杂毛 (role=spec)'; }
      else if (role) { roleKey = 'misc'; roleLabel = role; roleTitle = `角色: ${role}`; }
      // R24 (2026-07-26): 板块联动强度 — zt_today + zt_recent + pct 综合
      const ztToday = r.zt_today ? 1 : 0;
      const ztRecent = Number(r.zt_recent) || 0;
      let synScore = 0;
      if (ztToday) synScore += 5;
      synScore += Math.min(ztRecent, 5) * 2;
      if (pct >= 7) synScore += 2;
      else if (pct >= 3) synScore += 1;
      else if (pct <= -5) synScore -= 1;
      let synCls = 'none', synLabel = '弱', synTitle = `联动 ${synScore}`;
      if (synScore >= 8) { synCls = 'hot';    synLabel = '🔥'; synTitle = `强联动 score=${synScore} (今日涨停${ztToday ? '+' : '-'} + 近 3 日 ${ztRecent} 次涨停)`; }
      else if (synScore >= 5) { synCls = 'warm';   synLabel = '温'; synTitle = `中等联动 score=${synScore}`; }
      else if (synScore >= 2) { synCls = 'cool';   synLabel = '冷'; synTitle = `弱联动 score=${synScore}`; }
      else { synCls = 'none'; synLabel = '—'; synTitle = `无联动 score=${synScore}`; }
      // R25 (2026-07-26): 风险信号 — 高振幅 + 低市值 + 高 PE 综合
      const amp = Number(r.amplitude) || 0;
      const mcap = Number(r.mcap_yi) || 0;
      const pe = Number(r.pe_ttm) || 0;
      const volRatio = Number(r.volume_ratio) || 0; /* R-fix (2026-07-26): 上提 volRatio,被 riskScore + 量价共用,避免 TDZ */
      let riskScore = 0;
      if (amp >= 8) riskScore += 2; else if (amp >= 5) riskScore += 1;
      if (mcap > 0 && mcap < 50) riskScore += 2; else if (mcap > 0 && mcap < 100) riskScore += 1;
      if (pe > 200) riskScore += 1;
      if (volRatio >= 3) riskScore += 1;
      let riskCls = 'none', riskLabel = '低', riskTitle = `风险 ${riskScore}`;
      if (riskScore >= 4) { riskCls = 'high'; riskLabel = '⚠'; riskTitle = `高风险 score=${riskScore} (振幅 ${amp.toFixed(1)}% / 市值 ${mcap.toFixed(0)}亿 / PE ${pe.toFixed(0)})`; }
      else if (riskScore >= 2) { riskCls = 'mid'; riskLabel = '中'; riskTitle = `中风险 score=${riskScore}`; }
      else { riskCls = 'low'; riskLabel = '低'; riskTitle = `低风险 score=${riskScore}`; }
      // R26 (2026-07-26): 趋势信号 — 5d/20d 综合判断 (强/升/震/杀)
      const pct5d = Number(r.pct_5d) || 0;
      const pct20d = Number(r.pct_20d) || 0;
      let trendScore = 0;
      if (pct5d >= 5) trendScore += 2; else if (pct5d >= 2) trendScore += 1; else if (pct5d <= -5) trendScore -= 1;
      if (pct20d >= 15) trendScore += 2; else if (pct20d >= 5) trendScore += 1; else if (pct20d <= -15) trendScore -= 1;
      let trendCls = 'sideways', trendLabel = '震', trendTitle = `趋势 score=${trendScore} (5d ${pct5d.toFixed(1)}% / 20d ${pct20d.toFixed(1)}%)`;
      if (trendScore >= 3)       { trendCls = 'strong';   trendLabel = '强'; trendTitle = `强势趋势 score=${trendScore}`; }
      else if (trendScore >= 1)  { trendCls = 'uptrend';  trendLabel = '升'; trendTitle = `上行趋势 score=${trendScore}`; }
      else if (trendScore <= -2) { trendCls = 'down';     trendLabel = '杀'; trendTitle = `杀跌趋势 score=${trendScore}`; }
      // R27 (2026-07-26): 龙头属性 — main+大市值=👑 crown / 明星=⭐ leader / 跟=follower
      let leadCls = 'none', leadLabel = '—', leadTitle = '无龙头属性';
      const isMainRole = tax.role === 'main';
      const isHighMcap = mcap > 0 && mcap >= 200;
      if (isMainRole && isHighMcap) { leadCls = 'crown'; leadLabel = '👑'; leadTitle = '板块绝对龙头 (main + 大市值)'; }
      else if (isMainRole || (ztToday && ztRecent >= 1)) { leadCls = 'leader'; leadLabel = '⭐'; leadTitle = `板块明星 role=${tax.role || 'none'} ztToday=${ztToday} ztRecent=${ztRecent}`; }
      else if (pct >= 3 && pct <= 7 && ztRecent >= 1) { leadCls = 'follower'; leadLabel = '跟'; leadTitle = '跟随龙头 (温和涨 + 近期有涨停)'; }
      // 板块 = L2 SW,单个 chip,单击筛选
      const secChip = tax.l2
        ? `<span class="chip chip-l2 chip-click chip-trunc" data-goto-l2="${escapeHtml(tax.l2)}" title="板块 · ${escapeHtml(tax.l2)}">${escapeHtml(tax.l2)}</span>`
        : '<span class="dim">—</span>';
      ...
      const conceptChip = conceptPrimary
        ? `<span class="chip chip-l3 chip-click chip-trunc" data-goto-l3="${escapeHtml(conceptPrimary)}" title="概念 · ${escapeHtml([tax.l3, ...l4List].filter(Boolean).join(' / '))}">${escapeHtml(conceptPrimary)}${conceptExtra > 0 ? `<sup class="chip-more">+${conceptExtra}</sup>` : ''}</span>`
        : '<span class="dim">—</span>';
      // R21 (2026-07-26): 量价配合信号 — 涨幅 × 量比 综合判断
      let vpCls = 'flat', vpLabel = '—', vpSig = '0', vpTitle = '无量价异动';
      if (pct >= 7 && volRatio >= 1.5) { vpCls = 'strong'; vpLabel = '强'; vpSig = '3'; vpTitle = `放量涨 ${pct.toFixed(1)}% / 量比 ${volRatio.toFixed(1)}`; }
      else if (pct >= 2 && volRatio >= 1.2) { vpCls = 'good'; vpLabel = '好'; vpSig = '2'; vpTitle = `温和放量涨 ${pct.toFixed(1)}% / 量比 ${volRatio.toFixed(1)}`; }
      else if (pct <= -7 && volRatio >= 1.5) { vpCls = 'bad'; vpLabel = '弱'; vpSig = '1'; vpTitle = `放量跌 ${pct.toFixed(1)}% / 量比 ${volRatio.toFixed(1)} — 警惕出货`; }
      else if (volRatio >= 2.5) { vpCls = 'abnormal'; vpLabel = '异'; vpSig = '1'; vpTitle = `异常放量 量比 ${volRatio.toFixed(1)}`; }
      else if (pct <= -2 && volRatio < 0.6) { vpCls = 'wash'; vpLabel = '洗'; vpSig = '0'; vpTitle = `缩量跌 ${pct.toFixed(1)}% — 可能是洗盘`; }
      else if (pct >= 2 && volRatio < 0.6) { vpCls = 'tame'; vpLabel = '温'; vpSig = '1'; vpTitle = `缩量涨 ${pct.toFixed(1)}%`; }

      const ztTag = r.zt_today
        // 100-R5: 连板天 pill — 5 级 (L1=首板 / L2=2 连 / L3=3 连 / L4=4 连 / L5+=5+ 连)
        ? (() => {
            const streak = r.zt_streak || 1;
            const tier = streak >= 5 ? 'zt-l5' : streak === 4 ? 'zt-l4' : streak === 3 ? 'zt-l3' : streak === 2 ? 'zt-l2' : 'zt-l1';
            return `<span class="zt-tag ${tier}" title="${streak} 连板"><span class="zt-icon"></span>涨停 · L${streak >= 5 ? '5+' : streak}</span>`;
          })()
        : r.zt_recent ? `<span class="zt-recent" title="近 3 日累计涨停 ${r.zt_recent} 次">${r.zt_recent}日</span>`
        : '<span class="dim">—</span>';
      return `<tr class="stock-row" data-code="${escapeHtml(r.code)}" data-name="${escapeHtml(r.name || '')}" data-trend="${trendScore}" data-lead="${leadCls}" data-role="${roleKey}" data-risk="${riskScore}" data-synergy="${synScore}" data-vp-sig="${vpSig}">
        ...
        <td class="num" data-priority="3" data-col="风险" data-risk="${riskScore}" title="${riskTitle}"><span class="risk-badge ${riskCls}">${riskLabel}</span></td>
        <td class="num" data-priority="2" data-col="联动" data-synergy="${synScore}" title="${synTitle}"><span class="syn-badge ${synCls}">${synLabel}</span></td>
        <td class="num" data-priority="2" data-col="量价" data-vp-sig="${vpSig}" title="${vpTitle}"><span class="vp-badge ${vpCls}">${vpLabel}</span></td>
        ...
        <td class="cat-mid" data-priority="3" data-col="角色" data-role="${roleKey}" title="${roleTitle}"><span class="role-badge ${roleKey}">${roleLabel}</span></td>
        ...
        <td class="cat-mid" data-priority="3" data-col="趋势" data-trend="${trendScore}" title="${trendTitle}"><span class="trend-badge ${trendCls}">${trendLabel}</span></td>
        <td class="cat-mid" data-priority="3" data-col="龙头属性" data-lead="${leadCls}" title="${leadTitle}"><span class="lead-badge ${leadCls}">${leadLabel}</span></td>
        <td class="cat-mid" data-priority="3" data-col="得鑫" data-dxin-code="${escapeHtml(r.code)}"><span class="dxin-row-badge" data-dxin-pending title="点击行触发得鑫量变术 5 阶段判定">验</span></td>
      </tr>`;
    }).join('');
    // R8-perf: 记录追加前行数,使 refreshStarMarks 只扫新增行
    const prevStarCount = append ? document.querySelectorAll('#as-stocks-tbody .star-btn').length : 0;
    ...
    if (typeof refreshStarMarks === 'function') refreshStarMarks(prevStarCount);
    // 2026-07-14: 同链涨停 chips — 触发批量拉取(防重复入队)
    hydrateZtChainChips();
    // 100-R3: 5D sparkline 批量拉取(单次 ≤80 只,与列渲染解耦)
    hydrateSparklines();
    // R20: 得鑫列点击 — 走 DexinTrendAgent 单股判定 (modal)
    hydrateDexinBadges();
    // v250 (2026-07-30): 移动端卡片化渲染 — ≤768 显示,≥769 隐藏
    renderStockCards(items, append);
  }

  // === v250 — 全 A 风向移动端卡片化 =====================================
  // 仿龙头页 .dragon-card: rank/code/name/涨幅/板块/概念/换手/成交/市值 + 自选
  // 数据源 = 表格 items,sparkline chips / zt-chips 复用表格 hydrate 结果
  function renderStockCards(items, append) {
    const list = $('#as-cards-list');
    if (!list) return;
    const html = items.map(r => _renderStockCard(r)).join('');
    if (append) list.insertAdjacentHTML('beforeend', html);
    else list.innerHTML = html;
    // 一次绑定: 卡片 click 跳个股页 (除了 .star-btn / .chip)
    if (!list.dataset.bound) {
      list.dataset.bound = '1';
      list.addEventListener('click', (e) => {
        if (e.target.closest('.star-btn')) return;  // 自选独立处理
        if (e.target.closest('.chip')) {
          // chip = 跳板块筛选 (跟表格一致: data-goto-l2/l3/domain)
          const chip = e.target.closest('.chip');
          const l2 = chip.getAttribute('data-goto-l2');
          const l3 = chip.getAttribute('data-goto-l3');
          const l4 = chip.getAttribute('data-goto-l4');
          const dom = chip.getAttribute('data-goto-domain');
          if (l2 && state.l2 !== l2) { state.l2 = l2; syncUI(); syncUrl(); _debouncedBoardLoad(); return; }
          if (l3 && state.l3 !== l3) { state.l3 = l3; syncUI(); syncUrl(); _debouncedBoardLoad(); return; }
          if (l4 && state.l4 !== l4) { state.l4 = l4; syncUI(); syncUrl(); _debouncedBoardLoad(); return; }
          if (dom && state.domain !== dom) { state.domain = dom; syncUI(); syncUrl(); _debouncedBoardLoad(); return; }
          return;
        }
        const card = e.target.closest('.as-stock-card');
        if (!card) return;
        const code = card.dataset.code;
        const name = card.dataset.name || '';
        if (typeof gotoStock === 'function') gotoStock(code, name);
        else if (typeof window.gotoStock === 'function') window.gotoStock(code, name);
        else location.hash = '#stock/' + code;
      });
    }
    // 自选按钮 (避免渲染时单独 querySelectorAll — 复用全局 refreshStarMarks)
    if (typeof refreshStarMarks === 'function') {
      // 移动端卡片上的 star-btn 也归 refreshStarMarks 管 (类名一致)
      refreshStarMarks(append ? document.querySelectorAll('#as-cards-list .star-btn').length - items.length : 0);
    }
    // 更新"已选 X 项"计数 (filter hint)
    _updateAsFilterCount();
  }

  function _renderStockCard(r) {
    const pct = Number(r.change_pct) || 0;
    const pctCls = pct > 0 ? 'up' : pct < 0 ? 'down' : '';
    const pctTier =
      pct >= 7  ? 'pct-tier-deep-up'   :
    ...
    const amtYi = Number(r.amount_yi) || 0;
    const mcap = Number(r.mcap_yi) || 0;
    const tax = r.taxonomy || {};
    const role = tax.role || '';
    const roleMap = { main: '👑龙头', sub: '二线', spec: '杂毛' };
    const roleLabel = roleMap[role] || '—';
    const sector = tax.l2 || '';
    const l3 = tax.l3 || '';
    const ztToday = r.zt_today ? 1 : 0;
    const streak = r.zt_streak || 1;
    const ztTier = streak >= 5 ? 'zt-l5' : streak === 4 ? 'zt-l4' : streak === 3 ? 'zt-l3' : streak === 2 ? 'zt-l2' : 'zt-l1';
    const ztHtml = ztToday
      ? `<span class="as-card-zt ${ztTier}" title="${streak} 连板">涨停${streak >= 5 ? '5+' : streak >= 2 ? '·L' + streak : ''}</span>`
      : r.zt_recent ? `<span class="as-card-ztrecent" title="近 3 日累计涨停 ${r.zt_recent} 次">${r.zt_recent}日</span>` : '';
    return `<div class="as-stock-card" data-code="${escapeHtml(r.code)}" data-name="${escapeHtml(r.name || '')}" data-role="${escapeHtml(role)}">
      <div class="as-card-head">
        <span class="star-btn" data-star-code="${escapeHtml(r.code)}" data-star-name="${escapeHtml(r.name || '')}" title="加自选"></span>
        <span class="as-card-code">${escapeHtml(r.code)}</span>
        <span class="as-card-name" title="${escapeHtml(r.name || '')}">${escapeHtml(r.name || '')}</span>
        <span class="as-card-pct ${pctCls} ${pctTier}">${pct >= 0 ? '+' : ''}${pct.toFixed(2)}%</span>
      </div>
      <div class="as-card-meta">
        ${sector ? `<span class="chip chip-click chip-trunc as-card-sector" data-goto-l2="${escapeHtml(sector)}" title="板块 · ${escapeHtml(sector)}">${escapeHtml(sector)}</span>` : '<span class="dim">—</span>'}
        ${l3 ? `<span class="chip chip-click chip-trunc as-card-concept" data-goto-l3="${escapeHtml(l3)}" title="概念 · ${escapeHtml(l3)}">${escapeHtml(l3)}</span>` : ''}
        <span class="as-card-num" title="换手率">换 ${turn.toFixed(1)}%</span>
        <span class="as-card-num" title="成交额">额 ${amtYi.toFixed(1)}亿</span>
        <span class="as-card-num" title="市值">值 ${mcap >= 10000 ? (mcap / 10000).toFixed(2) + '万亿' : mcap.toFixed(0) + '亿'}</span>
        ${roleLabel !== '—' ? `<span class="as-card-role as-role-${role}">${roleLabel}</span>` : ''}
        ${ztHtml}
      </div>
    </div>`;
  }

  function _updateAsFilterCount() {
    const cnt = (state.l1 ? 1 : 0) + (state.l2 ? 1 : 0) + (state.l3 ? 1 : 0)
              + (state.l4 ? 1 : 0) + (state.domain ? 1 : 0)
              + state.presets.size + state.signals.size;
    const el = $('#as-filter-count');
    if (el) el.textContent = String(cnt);
  }

  // 100-R3: 5D sparkline — 单次 POST /api/stock/sparklines 拉 ≤80 行,避免 N×RTT
  // 用 data-spark-hydrated 标记已渲染,防止滚动追加时重复拉
  let _sparkHydrateTimer = null;
  let _sparkInflight = null;
  // R20: 得鑫列 — 行级点击委派给 dexinCheckOne, 见底全局 _dexinCheckDelegate
  function hydrateDexinBadges() {
    const cells = document.querySelectorAll('#as-stocks-tbody td[data-dxin-code]:not([data-dxin-bound])');
    if (!cells.length) return;
    cells.forEach(td => {
      td.dataset.dxinBound = '1';
      td.style.cursor = 'pointer';
      td.addEventListener('click', (e) => {
        e.stopPropagation();
        e.preventDefault();
        const code = td.dataset.dxinCode;
        if (code && typeof dexinCheckOne === 'function') {
          dexinCheckOne(code);
        }
      });
    });
  ...
  function hydrateSparklines() {
    if (_sparkHydrateTimer) clearTimeout(_sparkHydrateTimer);
    _sparkHydrateTimer = setTimeout(async () => {
      _sparkHydrateTimer = null;
      const cells = Array.from(document.querySelectorAll('#as-stocks-tbody td[data-spark-code]:not([data-spark-hydrated])'));
      if (!cells.length) return;
      // 截断 ≤80 避免单次 payload 太大
      const target = cells.slice(0, 80);
      target.forEach(c => c.dataset.sparkHydrated = 'pending');
      const codes = target.map(c => c.dataset.sparkCode).filter(Boolean);
      if (_sparkInflight) {
        try { await _sparkInflight; } catch (e) {}
      }
      _sparkInflight = (async () => {
        try {
          const resp = await fetch('/api/stock/sparklines', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
          ...
          for (const cell of target) {
            const code = cell.dataset.sparkCode;
            const rows = data[code];
            cell.innerHTML = renderSparkSvg(rows || []);
            cell.dataset.sparkHydrated = '1';
          }
        } catch (e) {
          // 失败时标 done,避免无限重试 (用户可以手动 refresh)
          for (const cell of target) {
            cell.innerHTML = '<span class="dim" style="font-size:10px">·</span>';
            cell.dataset.sparkHydrated = '1';
          }
        } finally {
          _sparkInflight = null;
      ...
      await _sparkInflight;
    }, 120);
  }

  // 100-R3 + R61 (2026-07-26): 60×16 升 80×22,加 5 点圆点 + 渐变 area 填充 (涨绿跌红半透)
  function renderSparkSvg(rows) {
    if (!rows || rows.length < 2) return '<span class="dim" style="font-size:10px">—</span>';
    const closes = rows.map(r => +r.close).filter(c => c > 0);
    if (closes.length < 2) return '<span class="dim" style="font-size:10px">—</span>';
    ...
    const fillD = `${d} L${lastPt[0].toFixed(1)},${H - PAD} L${PAD},${H - PAD} Z`;
    return `<svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" style="display:block;margin:0 auto;">
      ...
      ${pts.map((p, i) => `<circle cx="${p[0].toFixed(1)}" cy="${p[1].toFixed(1)}" r="${i === pts.length - 1 ? 1.8 : 1}" fill="${color}"/>`).join('')}
    </svg>`;
  }
  ...
  function setupTbodyDelegate() {
    ...
    // R33-R34 (2026-07-26): 键盘快捷键 — J/K 行导航 / Enter 跳转 / E 加自选 / F 折叠筛选
    // 状态: _focusedRowIndex (聚焦行) + _focusedRow (聚焦元素)
    function _refreshFocusedRow() {
      const rows = $$('#as-stocks-tbody tr.stock-row');
      rows.forEach(r => r.classList.remove('focused-row'));
      const idx = Math.min(Math.max(state._focusedRowIndex || 0, 0), rows.length - 1);
      if (rows[idx]) {
        rows[idx].classList.add('focused-row');
        state._focusedRow = rows[idx];
        state._focusedRowIndex = idx;
        rows[idx].scrollIntoView({ block: 'nearest', behavior: 'smooth' });
      }
    }
    state._focusedRowIndex = state._focusedRowIndex || 0;
    document.addEventListener('keydown', (e) => {
      // 只在 all_stocks view 启用
      if (!_currentViewName || _currentViewName() !== 'all_stocks') return;
      // 忽略输入控件焦点
      ...
      if (e.key === 'j' || e.key === 'J' || e.key === 'ArrowDown') {
        e.preventDefault();
        state._focusedRowIndex = Math.min((state._focusedRowIndex || 0) + 1, rows.length - 1);
        _refreshFocusedRow();
      } else if (e.key === 'k' || e.key === 'K' || e.key === 'ArrowUp') {
        e.preventDefault();
        state._focusedRowIndex = Math.max((state._focusedRowIndex || 0) - 1, 0);
        _refreshFocusedRow();
      } else if (e.key === 'Enter' && state._focusedRow) {
        e.preventDefault();
        const code = state._focusedRow.dataset.code;
        if (code) gotoStock(code);
      } else if (e.key === 'e' || e.key === 'E') {
        e.preventDefault();
        const row = state._focusedRow || rows[state._focusedRowIndex || 0];
        if (row) { const star = row.querySelector('.star-btn'); if (star) star.click(); }
      } else if (e.key === '/') {
        e.preventDefault();
        const search = $('#as-quick') || $('input[type="search"]');
        if (search) { search.focus(); search.select(); }
      } else if (e.key === 'f' || e.key === 'F') {
        e.preventDefault();
        const ft = $('#as-filter-toggle');
        if (ft) ft.click();
      } else if (e.key === 'Escape') {
        // 清除聚焦
        if (state._focusedRow) {
          state._focusedRow.classList.remove('focused-row');
          state._focusedRow = null;
        }
      }
    });
    // 视图首次进入 + 重渲染时刷新聚焦状态
    setTimeout(_refreshFocusedRow, 200);
    // R31 (2026-07-26): 双击行 = 一键加自选 (toggle),用于 touchpad/外接键盘
    // 修法:延迟到 dblclick 后调 wlToggle (single click 已触发 gotoStock,但 dblclick 后我们 navigate 接管被消除)
    // 实际上 dblclick 会触发 2 次 click + 1 次 dblclick;2 次 click 互相到 gotoStock,2 次触发 gotoStock 同页 = OK
    tbody.addEventListener('dblclick', (e) => {
      const row = e.target.closest('tr.stock-row');
      if (!row || !row.dataset.code) return;
      if (e.target.closest('.star-btn, button, .chip, [data-goto-l2], [data-goto-l3], [data-goto-l4], [data-goto-domain], .code-link')) return;
      // 阻止默认 dblclick 文字选择
      e.preventDefault();
      const code = row.dataset.code;
      const name = row.dataset.name || '';
      const star = row.querySelector('.star-btn');
      if (star) star.click();  // 走 wlToggle 完整路径
      else if (window.wlToggle) window.wlToggle(code, name);
    });
    // R32 (2026-07-26): 长按 600ms = 弹出迷你 K线预览 (fix row click 600ms hold 不再算 single click)
    let _pressTimer = null, _pressStartY = 0, _pressTarget = null, _pressStartX = 0;
    tbody.addEventListener('touchstart', (e) => {
      ...
      _pressStartX = e.touches[0].clientX;
      _pressTimer = setTimeout(() => { _showKlinePreview(row); _pressTarget = null; }, 600);
    }, { passive: true });
    tbody.addEventListener('touchmove', (e) => {
      if (!_pressTarget) return;
      const dy = Math.abs(e.touches[0].clientY - _pressStartY);
      const dx = Math.abs(e.touches[0].clientX - _pressStartX);
      if (dy > 12 || dx > 12) { clearTimeout(_pressTimer); _pressTarget = null; }
    }, { passive: true });
    tbody.addEventListener('touchend', () => { if (_pressTimer) clearTimeout(_pressTimer); _pressTarget = null; });
    tbody.addEventListener('touchcancel', () => { if (_pressTimer) clearTimeout(_pressTimer); _pressTarget = null; });
    ...
    tbody.addEventListener('click', async (e) => {
        ...
        if (isActive) {
          ...
          try {
            ...
            if (env.ok) {
              ...
              toast('✗ 加入失败: ' + (env.error || '未知'), 'error', 3000);
            }
          } catch (err) {
            toast('✗ 加入失败: ' + err.message, 'error', 3000);
          }
      ...
      // 代码 → 个股详情 (走主 app gotoStock, 触发 showView('stock') + AI 复盘)
      const codeLink = e.target.closest('.code-link');
      if (codeLink && codeLink.dataset.code) {
        e.stopPropagation();
        gotoStock(codeLink.dataset.code);
        return;
      }
      // 行空白处 → 也跳转个股 (沿用整行点击)
      const row = e.target.closest('tr.stock-row');
      if (row && row.dataset.code && !e.target.closest('button, .star-btn, .chip, [data-goto-l2], [data-goto-l3], [data-goto-l4], [data-goto-domain], .code-link, .zt-chips-placeholder')) {
        gotoStock(row.dataset.code);
        return;
      }
  ...
  // R32 (2026-07-26): 长按触发的迷你 K线预览 — 显示 30 行 popover 含价格/涨跌幅/3 关键位
  function _showKlinePreview(row) {
    const code = row.dataset.code;
    const name = row.dataset.name || '';
    const price = row.querySelector('[data-col="涨幅"]')?.innerText || '—';
    const pct = row.querySelector('[data-col="换手"]')?.innerText || '—';
    // 移除旧预览
    document.getElementById('kline-preview-popover')?.remove();
    const pop = document.createElement('div');
    pop.id = 'kline-preview-popover';
    pop.className = 'kline-preview-popover';
    pop.innerHTML = `
      <div class="klpv-head">
        <span class="klpv-name">${escapeHtml(name)}</span>
        <span class="klpv-code">${escapeHtml(code)}</span>
        <button class="klpv-close" type="button">×</button>
      </div>
      <div class="klpv-quick">
        <div><span class="dim">涨幅</span><b>${price}</b></div>
        <div><span class="dim">换手</span><b>${pct}%</b></div>
      </div>
      <div class="klpv-actions">
        <button class="klpv-go" type="button">📈 查看完整 K 线</button>
        <button class="klpv-add" type="button">⭐ 加自选</button>
      </div>
      <div class="klpv-hint">长按触发的快速预览 · 释放手指前往</div>
    `;
    document.body.appendChild(pop);
    // 定位
    const r = row.getBoundingClientRect();
    const left = Math.min(r.right + 8, window.innerWidth - 320);
    const top = Math.max(8, r.top - 4);
    pop.style.left = `${left}px`;
    pop.style.top = `${top}px`;
    pop.querySelector('.klpv-close').onclick = () => pop.remove();
    pop.querySelector('.klpv-go').onclick = () => { pop.remove(); gotoStock(code); };
    pop.querySelector('.klpv-add').onclick = () => {
      pop.remove();
      const star = row.querySelector('.star-btn');
      if (star) star.click();
    };
  ...
  // R-2026-07-16: Hero KPI strip (#as-kpis) — 与 screener/watchlist 视觉对齐
  // 5 块 kpi-group: 总数 / 涨家 / 跌家 / 涨停 / 主力净流入;统计基于完整过滤集 (来自后端 stats)
  // R-100-R1: 每卡前缀内联 SVG 图标,主题色染色 — 信息密度↑,视觉层级清晰
  function renderAsKpis(stats) {
    const host = $('#as-kpis');
    if (!host) return;
    if (!stats) { host.innerHTML = ''; return; }
    // R1-25 (2026-07-17): 基于"已聚合 / 全候选"透明披露 — stats_source_count 是实时统计的,
    // stats_total_count 是全 universe/filter 后的候选总数。让用户清楚统计基于多少只
    const sourceCnt = stats.stats_source_count || 0;
    const totalCnt  = stats.stats_total_count || state.totalCandidates || state.totalAvailable || 0;
    const coveragePct = totalCnt > 0 ? (sourceCnt / totalCnt * 100).toFixed(0) : 0;
    const coverageLow = sourceCnt > 0 && sourceCnt < totalCnt * 0.8;
    const totalAvail = state.totalCandidates || state.totalAvailable || 0;
    ...
    const ICO = {
      ...
      up:    '<svg class="kpi-ico" viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"><path d="M3 11l4-5 3 3 3-6"/><path d="M10 3h3v3"/></svg>',
      zt:    '<svg class="kpi-ico" viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M8 2l1.5 4.5L14 8l-4.5 1.5L8 14l-1.5-4.5L2 8l4.5-1.5z"/></svg>',
      fund:  '<svg class="kpi-ico" viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="8" cy="8" r="6"/><path d="M5 8h6M8 5v6"/></svg>',
      mcap:  '<svg class="kpi-ico" viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M2 13l3-4 3 2 5-7"/><path d="M2 14h12"/></svg>',
    };
    const fundKpi = `<div class="kpi-group"><span class="kpi-label">${ICO.fund}主力净流入</span><span class="kpi-value" style="color:${fundColor}">${fundStr}</span><span class="kpi-sub">${fund >= 0 ? '+' : ''}${fund.toFixed(0)} 万 · 均涨 ${avgPct >= 0 ? '+' : ''}${avgPct.toFixed(2)}%</span></div>`;
    // 100-R2: 涨/平/跌 3 段比例水平条 — 视觉占比,信息密度↑↑
    const upN = stats.up || 0, flatN = stats.flat || 0, downN = stats.down || 0;
    const totalBars = Math.max(1, upN + flatN + downN);
    const upPct = (upN / totalBars * 100).toFixed(1);
    const flatPct = (flatN / totalBars * 100).toFixed(1);
    ...
    host.innerHTML = `
      <div class="kpi-group kpi-group-compact"><span class="kpi-label">${ICO.total}统计覆盖</span><span class="kpi-value">${sourceCnt}<span class="kpi-sep">/</span><span style="color:var(--ink-3);font-size:0.85em">${totalCnt}</span></span><span class="kpi-sub" style="${coverageLow ? 'color:var(--warn,var(--warn))' : ''}">${coverageLow ? `仅覆盖 ${coveragePct}%` : `${coveragePct}%`} · 加载 ${state.loadedCount || 0}</span></div>
  ...
  // 2026-07-16: 标题里加「板块名 · 涨跌%」chip
  // 选中 L1/L2/L3/L4/domain 任一时,把第一项塞进 <h1> 后面,显示当前过滤集的均涨%
  // 没选板块时清掉,回到默认「全 A 风向 5400+」
  function renderTitleSectorBadge(stats) {
    const el = $('#as-title-pct');
    if (!el) return;
    // 收集当前生效的 sector label (L1 > L2 > L3 > L4 > domain,优先级按层级)
    const pick = (val, prefix) => {
      const v = (val || '').split(',').filter(Boolean);
      return v.length ? { label: prefix + ' · ' + v[0], prefix } : null;
    };
    const candidates = [
      pick(state.l1, 'L1'),
      pick(state.l2, 'L2'),
      pick(state.l3, 'L3'),
      pick(state.l4, 'L4'),
      pick(state.domain, 'AI战场'),
    ].filter(Boolean);
    if (!candidates.length || !stats) {
      el.hidden = true;
      el.textContent = '';
  ...
  // R9-perf (2026-07-15): 仅筛选状态变化时重绘 active filters,避免每次 loadBoard 重建 DOM
  var _lastActiveFilterHash = '';
  function renderActiveFilters() {
    ...
    const hash = tags.map(t => t.key + ':' + t.val).join('|');
    if (hash === _lastActiveFilterHash) return;
    _lastActiveFilterHash = hash;
    ...
    // 点击移除芯片
    root.querySelectorAll('.chip-active .x').forEach(x => {
  ...
  function syncUI() {
    ...
    [['as-l2',state.l2],['as-l3',state.l3],['as-l4',state.l4],['as-domain',state.domain]].forEach(([id, val]) => {
      ...
      if (n > 0) {
        // R-2026-07-16: label 即 group (.as-fg), badge 直接挂在 .fg-label 旁边
        const fgLabel = group.querySelector('.fg-label');
        if (fgLabel && !fgLabel.querySelector('.count-badge')) {
          fgLabel.appendChild(el('span', { class: 'count-badge' }, String(n)));
        }
      }
    });
  }
  ...
  function bindSortHeader() {
    $$('#as-stocks-table thead th.sortable').forEach(th => {
      th.addEventListener('click', () => {
        ...
        syncUrl();
        // R26-27 (2026-07-26): 派生列 (trend/lead/risk/synergy/vp_sig/role) 走 client sort — 避免打 server
        if (_DERIVED_SORT_KEYS.has(sort)) {
          _clientSortDerived(sort, order);
          updateSortArrows();
        } else {
          _debouncedBoardLoad();
          updateSortArrows();
  ...
  // R26-27 (2026-07-26): 派生列 client sort — 已渲染行的 data-* 属性排序,不打 server
  const _DERIVED_SORT_KEYS = new Set(['trend', 'lead', 'risk', 'synergy', 'vp_sig', 'role']);
  const _DERIVED_SORT_ATTR = { trend: 'trend', lead: 'lead', risk: 'risk', synergy: 'synergy', vp_sig: 'vpSig', role: 'role' };
  function _clientSortDerived(sortKey, order) {
    const tbody = $('#as-stocks-tbody');
    if (!tbody) return;
    const attr = _DERIVED_SORT_ATTR[sortKey] || sortKey;
    const rows = Array.from(tbody.querySelectorAll('tr.stock-row'));
    rows.sort((a, b) => {
      // lead 用文字排序 (crown>leader>follower>none)
      if (sortKey === 'lead') {
        const order = { crown: 3, leader: 2, follower: 1, none: 0 };
        const va = order[a.dataset.lead] ?? 0;
        const vb = order[b.dataset.lead] ?? 0;
        return va - vb;
      }
      // role 排序 (main>sub>spec>misc>none)
      if (sortKey === 'role') {
        const order = { main: 4, sub: 3, spec: 2, misc: 1, none: 0 };
        const va = order[a.dataset.role] ?? 0;
        const vb = order[b.dataset.role] ?? 0;
        return va - vb;
      }
      const va = Number(a.dataset[attr]) || 0;
      const vb = Number(b.dataset[attr]) || 0;
      return va - vb;
    });
    // 倒序时翻转
    if (order !== 'asc') rows.reverse();
    // 重新 append (detach 后 batch insert 减少 reflow)
  ...
  function updateSortArrows() {
    $$('#as-stocks-table thead th.sortable').forEach(th => {
      ...
      // R43 (2026-07-26): 加 sort-asc / sort-desc 类名让 CSS 强制 opacity=1
      th.classList.toggle('sort-asc',  isActive && state.order === 'asc');
      th.classList.toggle('sort-desc', isActive && state.order === 'desc');
      if (!arrow) return;
      if (isActive) {
  ...
  function bindControls() {
    ...
    // v250 (2026-07-30): 移动端筛选折叠 toggle — 重做,跟 #as-filter-body / #as-filter-hint 配合
    const filterToggle = $('#as-filter-toggle');
    const filterStrip = $('#as-filter-strip');
    const filterBody = $('#as-filter-body');
    const filterHint = $('#as-filter-hint');
    if (filterToggle && filterStrip) {
      const _applyMobileFilterToggle = () => {
        if (window.innerWidth <= 768) {
          // 移动端:折叠,显示 toggle + hint,隐藏 body
          filterStrip.dataset.collapsed = '1';
          ...
          // 桌面端:展开,隐藏 toggle + hint,显示 body
          filterStrip.dataset.collapsed = '0';
          filterToggle.classList.remove('active');
      ...
      _applyMobileFilterToggle();
      window.addEventListener('resize', _applyMobileFilterToggle);

      filterToggle.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        const isOpen = filterStrip.dataset.collapsed === '0';
        if (isOpen) {
          filterStrip.dataset.collapsed = '1';
          filterToggle.classList.remove('active');
          if (filterBody) filterBody.style.display = 'none';
    ...
    if (resetBtn) resetBtn.addEventListener('click', () => {
      ...
      state.offset = 0;
      // R38: 重置列显隐
      state._hiddenCols = new Set();
      try { localStorage.removeItem('all_stocks_hidden_cols'); } catch (_) {}
      // 同时清掉快速搜索框 + 触发行过滤
      if (qsInput) {
        qsInput.value = '';
    ...
    if (fab && scrollEl) {
      ...
      // R35: passive scroll listener — 不阻塞主线程滚动合成
  ...
  // === 18. applyQuickSearch ===============================================
  // R6-perf (2026-07-15): 用 classList + CSS class 替代 style.display,减少逐条 reflow
  function applyQuickSearch(q) {
    ...
    for (let i = 0, n = rows.length; i < n; i++) {
      ...
      tr.classList.toggle('qs-hidden', !show);
      if (show) shown++;
    }
  ...
  // R2-perf (2026-07-15): CSS class 列显隐,避免逐个设 style.display
  function applyColVisibility() {
    const hideSet = state._hiddenCols;
    const cells = document.querySelectorAll('#as-stocks-table thead th[data-col], #as-stocks-table tbody td[data-col]');
    for (let i = 0, n = cells.length; i < n; i++) {
      const col = cells[i].dataset.col;
      if (col && hideSet.has(col)) cells[i].classList.add('col-hidden');
      else cells[i].classList.remove('col-hidden');
    }
  }
  ...
  // R3-perf (2026-07-15): 增量 — append 时 skipCount 跳过已有行,只扫新增行
  function refreshStarMarks(skipCount) {
    const all = document.querySelectorAll('#as-stocks-tbody .star-btn');
    const start = skipCount || 0;
    for (let i = start, n = all.length; i < n; i++) {
      const el_ = all[i];
  ...
  function buildFilterSheetHTML() {
    return `
      ...
      <div class="sheet-section">
        <div class="sheet-label">主战场 (AI 战场)</div>
  ...
  function init() {
    ...
    Promise.all([
      ...
      updateSortArrows();
      _startAutoRefresh();
      // R33-R34 (2026-07-26): 首次加载 1s 后弹快捷键提示,6 秒自动消失
      if (!sessionStorage.getItem('as-shortcut-hint-shown')) {
        setTimeout(() => {
          _showShortcutHint();
          sessionStorage.setItem('as-shortcut-hint-shown', '1');
        }, 1200);
      }
  ...
  // R33-R34 (2026-07-26): 快捷键提示卡 — 一次性显示 6 秒,告知用户 J/K/Enter/E///F
  function _showShortcutHint() {
    const old = document.getElementById('shortcut-hint-card');
    if (old) old.remove();
    const card = document.createElement('div');
    card.id = 'shortcut-hint-card';
    card.className = 'shortcut-hint';
    card.innerHTML = `
      <b>J / K</b>下行/上行<br>
      <b>Enter</b> 跳转个股<br>
      <b>E</b> 一键加自选<br>
