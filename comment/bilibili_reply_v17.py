...
import sys  # 必须最先导入（fake bilibili_api 在它之前就需要 sys）
import socket  # 网络请求全局超时兜底
socket.setdefaulttimeout(10)
# 伪造 bilibili_api 模块，防止 exec(session.py) 时真实导入挂起
import types
_bili_api_fake = types.ModuleType('bilibili_api')
_bili_api_fake.__file__ = '/Users/kaikai/scripts/bilibili_api/__init__.py'
sys.modules['bilibili_api'] = _bili_api_fake
...
# 导入公共工具模块
sys.path.insert(0, '/Users/kaikai/scripts')  # 绝对路径，确保 bilibili_utils 可导入
try:
...
# MiniMax 配置
MINIMAX_API_KEY = "sk-cp-IDvom_ljx9fxR6B29s3eLyxIq0vcpcNR-n1NYv32WQCLXgYImxvMqYP3j0y2NOoA9qu_zMkEja4Ixwh3sLBGQYwE2P5kfe7e76Sgsaw8n-YXbomTpHGaE-4"
...
# ==================== 工具函数 ====================
def acquire_lock():
    # 清理 stale lock（上一个进程被 SIGKILL 时，来不及解锁）
    if LOCK_FILE.exists():
        try:
            old_pid = int(LOCK_FILE.read_text().strip())
            if old_pid != os.getpid() and old_pid > 0:
                try:
                    os.kill(old_pid, 0)  # 检查进程是否还活着
                except OSError:
                    # 进程已死，清理 stale lock
                    LOCK_FILE.unlink()
            else:
                LOCK_FILE.unlink()  # 自己的 lock 也清掉重新加
...
def timeout_handler(s, f):
    save_all()
    print("\n[v18] 运行超时（15分钟），强制退出", file=_real_stdout, flush=True)
    sys.exit(1)
...
# Cancel alarm when script exits normally
import atexit
@atexit.register
def _cancel_alarm():
    signal.alarm(0)
...
4. 如果之前聊过这个话题，要接着之前的内容继续，不要跑题或重新开头"""
    ...
    if reply is None:
        # 第一次失败可能是上下文超限，去掉history再试一次
        log("  ⚠️ 首次生成失败（上下文超限），去掉历史重试...")
        simple_prompt = f"""【任务】B站评论区回复，40-60字，幽默自然。

粉丝「{uname}」：{user_comment}

直接输出回复文字，不要分析。"""
        reply = call_minimax(simple_prompt)
        if reply is None:
...
def add_to_chat_history(aid: str, root_id: str, uname: str, user_comment: str, my_reply: str):
    ...
    # 保留最近 6 条消息（约3轮对话），避免上下文膨胀
    cache[key] = cache[key][-6:]
...
def build_conversation_prompt(uname: str, user_comment: str, chat_history: list, video_title: str = "", parent_comment: str = "") -> str:
    """
    精简prompt：去掉冗余格式，直接构建对话。
    最多包含3轮历史（约6条消息），控制token总量。
    """
    lines = []
    if video_title:
        lines.append(f"视频：{video_title}")
    if parent_comment and parent_comment != user_comment:
        lines.append(f"回复：「{parent_comment}」")
    lines.append(f"粉丝「{uname}」：{user_comment}")

    if chat_history:
        # 只取最近6条消息（3轮），减少token消耗
        for msg in chat_history[-6:]:
            role = msg.get("role", "user")
            content = msg.get("content", "")
            prefix = "你" if role == "assistant" else uname
            lines.append(f"{prefix}：{content[:80]}")  # 每条消息最多80字

    lines.append(f"你：")  # 留空让模型补全
    return "\n".join(lines)
...
def process_reply_messages():
    ...
    for page in range(1, 7):
        ...
        for item in items:
            ...
            retry_count = 0
            while retry_count < 2:
                ok = send_reply(subject_id, root, parent, reply_with_mention)
                if ok:
                    break
                retry_count += 1
                if retry_count >= 2:
                    break
                log(f"  ⏳ 发送失败，重试第{retry_count}次...")
...
# 15分钟硬性超时（脚本有90秒CD等待，正常运行可能超过5分钟）
signal.alarm(900)
