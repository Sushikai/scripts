...
def load_all_accounts():
    ...
    for i, path in enumerate(ALL_COOKIE_FILES):
        ...
        if not cookies.get("SESSDATA"):
            ...
            continue
        valid = _validate_account(cookies)
        if not valid:
...
ACCOUNTS = []
_valid_accounts = load_all_accounts()
for acc in _valid_accounts:
    acc["session"] = make_session()
    acc["replied_file"] = _INSTANCE_WORK / f"bili_replied_{acc['name'][:8]}.json" if _INSTANCE_WORK else Path(f"/tmp/bili_replied_{acc['name'][:8]}.json")
    acc["cooldown_file"] = _INSTANCE_WORK / f"bili_comment_cooldown_{acc['name'][:8]}.json" if _INSTANCE_WORK else Path(f"/tmp/bili_comment_cooldown_{acc['name'][:8]}.json")
    acc["log_file"] = _INSTANCE_WORK / f"bili_reply_v17_{acc['name'][:8]}.log" if _INSTANCE_WORK else Path(f"/tmp/bili_reply_v17_{acc['name'][:8]}.log")
    ACCOUNTS.append(acc)

for acc in ACCOUNTS:
    print(f"  已加载账号: {acc['name']} ({acc['sessdata'][:20]}...)")
...
def _load_cooldown(account: dict):
    try:
        if account["cooldown_file"].exists():
            account["cooldown_data"] = json.loads(account["cooldown_file"].read_text(encoding='utf-8'))
    except Exception:
        account["cooldown_data"] = {}

def _save_cooldown(account: dict):
    try:
        atomic_write(account["cooldown_file"], json.dumps(account["cooldown_data"], ensure_ascii=False, indent=2))
    except Exception:
        pass

def wait_for_cooldown(account: dict):
    _load_cooldown(account)
    last_ts = account["cooldown_data"].get('last_comment_ts', 0)
    elapsed = time.time() - last_ts
    if elapsed < MIN_COMMENT_INTERVAL:
        wait_time = MIN_COMMENT_INTERVAL - elapsed + random.uniform(1, 5)
        log(f"  ⏳ [{account['name']}] 等待冷却... ({wait_time:.1f}秒)")
...
def get_video_title(aid: str, account: dict) -> str:
    return title_cache.get(aid, account["session"], account["cookies"], HEADERS)

# ==================== 获取评论的父评论内容 ====================
def get_parent_comment(aid: str, parent_rpid: str, account: dict) -> str:
    if not parent_rpid or parent_rpid == '0':
        return ""
    for attempt in range(2):
        try:
            r = account["session"].get(
                f"https://api.bilibili.com/x/v2/reply?type=1&oid={aid}&ps=1&pn=1&root={parent_rpid}",
                headers=HEADERS, cookies=account["cookies"], timeout=8
            )
            data = r.json()
...
# ==================== 发送B站回复 ====================
def send_reply(oid: str, root: str, parent: str, content: str, account: dict) -> bool:
    wait_for_cooldown(account)
    ...
    for attempt in range(2):
        try:
            r = account["session"].post(
                ...
                headers=HEADERS, cookies=account["cookies"], timeout=10
            )
            j = r.json()
            if j.get('code') == 0:
                account["cooldown_data"]["last_comment_ts"] = time.time()
                _save_cooldown(account)
                return True
...
log("=== v18 Multi-Account MiniMax 精确回复版启动 ===")

# 除了主账号，其他账号1小时只能运行1次
PRIMARY_ACCOUNT = "20岁还没赚够100"

def _check_account_cooldown(acc: dict) -> bool:
    if PRIMARY_ACCOUNT in acc.get("name", ""):
        return True
    cd_file = acc.get("cooldown_file")
    if cd_file and cd_file.exists():
        try:
            last = json.loads(cd_file.read_text()).get("last_comment_ts", 0)
...
def _save_account_cooldown(acc: dict):
    if PRIMARY_ACCOUNT not in acc.get("name", ""):
        cd_file = acc.get("cooldown_file")
        if cd_file:
            try:
                cd_file.write_text(json.dumps({"last_comment_ts": time.time()}))
            except:
                pass
...
def process_reply_messages():
    total_sent = 0
    for acc in ACCOUNTS:
        if not _check_account_cooldown(acc):
            continue
        log(f"══ [{acc['name']}] 开始处理「回复我的」消息 ══")
        sent_count = 0
        processed = 0
        _store = {}
        try:
            if acc["replied_file"].exists():
                _store = json.loads(acc["replied_file"].read_text(encoding='utf-8'))
        except Exception:
            _store = {}
        ...
        for page in range(1, 7):
            try:
                r = acc["session"].get(
                    ...
                    headers=HEADERS, cookies=acc["cookies"], timeout=10
                )
                items = r.json().get('data', {}).get('items', [])
            except Exception as e:
                log(f"  [{acc['name']}] 获取第{page}页失败: {e}")
                continue
            ...
            title_cache.prefetch(aids, acc["session"], acc["cookies"], HEADERS)
            ...
            for item in items:
                ...
                video_title = get_video_title(subject_id, acc)
                parent_comment = get_parent_comment(subject_id, parent, acc) if parent != source_id else ""
                ...
                if send_reply(subject_id, root, parent, reply_with_mention, acc):
                    _store[source_id] = time.strftime('%Y-%m-%d %H:%M:%S')
                    atomic_write(acc["replied_file"], json.dumps(_store, ensure_ascii=False, indent=2))
        ...
        if sent_count > 0:
            _save_account_cooldown(acc)
        log(f"  [{acc['name']}] 本次扫描 {processed} 条消息，智能回复 {sent_count} 条")
